...
- 已补齐并落了最小实现：`NurbsCurve3d` / `NurbsSurface` / `RuledSurface` / `OffsetSurface` / `CurveOnSurface` / `BrepBody` 等第一阶段公开对象，以及 `GeometryMeasure` / `GeometryHealing` / `Validate(BrepBody, ...)` / `BrepFace -> TriangleMesh` / 受限 `BrepBody -> TriangleMesh` / `ConvertToPolyhedronFace` / `ConvertToPolyhedronBody` / `Section(BrepBody, Plane, ...)` / `Heal(BrepBody)` trim 回填入口；其中 `BrepFace` 已可沿 trim/UV 路径消费非平面面片
