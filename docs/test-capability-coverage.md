...
- `tests/capabilities/test_sdk.cpp`
  - SDK 层常用对象和基础 API 行为，以及 `LineCurve3d` / `PlaneSurface` / `CurveEval3d` / `SurfaceEval3d` / `TriangleMesh` / `MeshValidation3d` / `Tessellate(PlaneSurface, ...)` / `ConvertToTriangleMesh(PolyhedronFace3d / PolyhedronBody, ...)` / `ProjectFaceToPolygon2d(...)` / `PolyhedronLoop3d` / `PolyhedronFace3d` / `PolyhedronBody` / `PolyhedronValidation3d` 的最小 3D 能力
