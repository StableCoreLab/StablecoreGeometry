...
- `src/sdk/GeometryTopology.cpp`
  - `PolygonTopology2d::Build` 中，`Equal` 情况只允许更早出现的 polygon 作为 parent，避免 duplicate / equal case 互相争抢父节点
- `tests/test_topology_indexing.cpp`
  - 新增 duplicate-equal topology regression：`BuildPolygonTopology(MultiPolygon2d{outer, outer})`
- `docs/delphi-geometry-parity.md`
  - 已同步记录 equal duplicate polygon topology tie-break
...
- `test_relation_boolean.cpp`
  ...
  - touching difference
- `test_offset.cpp`
  - narrow bridge inward offset split into two polygons
- `test_topology_indexing.cpp`
  - equal relation
  - self-contains basic case
  - duplicate-equal topology parent tie-break
...
Must Close 剩余重点：
1. `SearchPoly` 风格更强的 branch scoring / fake-edge candidate ranking
2. boolean 在更复杂 collinear overlap family、重复边族、近退化交点下的进一步稳健性
3. offset 在 reverse-edge、self-intersection、多轮失败恢复、hole/outer 语义翻转上的进一步恢复
4. preprocessing / topology recovery 在 search / boolean / offset 之间进一步统一
5. relation hierarchy 向 `GGLPolyRelation` 再靠拢

建议优先级：
1. boolean 退化/重叠稳健性
2. offset 复杂恢复
3. SearchPoly 风格歧义候选路径决策
