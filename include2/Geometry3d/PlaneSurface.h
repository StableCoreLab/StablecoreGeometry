...
namespace Geometry
{
    ...
    public:
        ...
        [[nodiscard]] static PlaneSurface FromPlane( const Plane &plane,
        ...
        {
            if( !plane_.IsValid( tolerance.distanceEpsilon ) || !uRange_.IsValid() ||
                !vRange_.IsValid() )
            {
                return false;
            }

            if( uAxis_.Length() <= tolerance.distanceEpsilon ||
                vAxis_.Length() <= tolerance.distanceEpsilon )
            {
                return false;
            }
        ...
        [[nodiscard]] Intervald URange() const override { return uRange_; }
        ...
        [[nodiscard]] Plane SupportPlane() const { return plane_; }

        [[nodiscard]] Vector3d UAxis() const { return uAxis_; }

        [[nodiscard]] Vector3d VAxis() const { return vAxis_; }
