...
namespace Geometry
{
    ...
    public:
        ...
        OffsetSurface( std::shared_ptr<Surface> baseSurface, double offsetDistance ) :
            baseSurface_( std::move( baseSurface ) ),
        ...
        {
            return OffsetSurface( std::shared_ptr<Surface>( surface.Clone().release() ),
                                  offsetDistance );
        }

        [[nodiscard]] bool IsValid( const GeometryTolerance3d &tolerance = {} ) const override
        {
            return baseSurface_ != nullptr && baseSurface_->IsValid( tolerance ) &&
                   std::isfinite( offsetDistance_ );
        }
        ...
        {
            SurfaceEval3d eval = baseSurface_ != nullptr
                                     ? baseSurface_->Evaluate( u, v, derivativeOrder )
                                     : SurfaceEval3d{};
        ...
        [[nodiscard]] std::unique_ptr<Surface> Clone() const override
        {
            return std::make_unique<OffsetSurface>(
                baseSurface_ != nullptr ? std::shared_ptr<Surface>( baseSurface_->Clone().release() )
                                        : nullptr,
                offsetDistance_ );
        }

        [[nodiscard]] const Surface *BaseSurface() const { return baseSurface_.get(); }

        [[nodiscard]] double OffsetDistance() const { return offsetDistance_; }
