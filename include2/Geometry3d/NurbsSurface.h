...
namespace Geometry
{
    ...
    public:
        ...
        NurbsSurface( int degreeU, int degreeV, std::size_t controlPointCountU,
                      std::size_t controlPointCountV, std::vector<Point3d> controlPoints,
        ...
        {
            return IsValid() ? Intervald{ knotsU_[static_cast<std::size_t>( degreeU_ )],
                                          knotsU_[controlPointCountU_] }
                             : Intervald{};
        }

        [[nodiscard]] Intervald VRange() const override
        {
            return IsValid() ? Intervald{ knotsV_[static_cast<std::size_t>( degreeV_ )],
                                          knotsV_[controlPointCountV_] }
                             : Intervald{};
        }
        ...
        [[nodiscard]] SurfaceEval3d Evaluate( double u, double v,
                                              int derivativeOrder = 1 ) const override
        {
            ...
            if( derivativeOrder >= 1 )
            {
                const double du = std::max(
                    URange().Length() /
                        static_cast<double>( std::max<std::size_t>( controlPointCountU_ * 8, 8 ) ),
                    Geometry::kDefaultEpsilon );
                const double dv = std::max(
                    VRange().Length() /
                        static_cast<double>( std::max<std::size_t>( controlPointCountV_ * 8, 8 ) ),
                    Geometry::kDefaultEpsilon );
        ...
        [[nodiscard]] int DegreeU() const { return degreeU_; }

        [[nodiscard]] int DegreeV() const { return degreeV_; }

        [[nodiscard]] std::size_t ControlPointCountU() const { return controlPointCountU_; }

        [[nodiscard]] std::size_t ControlPointCountV() const { return controlPointCountV_; }

        [[nodiscard]] const std::vector<Point3d> &ControlPoints() const { return controlPoints_; }

        [[nodiscard]] const std::vector<double> &KnotsU() const { return knotsU_; }

        [[nodiscard]] const std::vector<double> &KnotsV() const { return knotsV_; }

    private:
        [[nodiscard]] static double Basis( const std::vector<double> &knots, std::size_t pointCount,
                                           std::size_t index, int degree, double parameter )
        {
            if( degree == 0 )
            {
                const bool inHalfOpen = knots[index] <= parameter && parameter < knots[index + 1];
                const bool isRightBoundary =
                    std::abs( parameter - knots[pointCount] ) <= Geometry::kDefaultEpsilon &&
                    index + 1 == pointCount;
            ...
            {
                right = ( ( knots[index + static_cast<std::size_t>( degree ) + 1] - parameter ) /
                          rightDenom ) *
