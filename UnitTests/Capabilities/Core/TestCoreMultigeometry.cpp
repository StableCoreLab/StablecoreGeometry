...
TEST( CoreMultiGeometryTest, CoversCurrentCapabilities )
{
    ...
    ASSERT_TRUE( ContainsPoint( outerPolygon, Point2d{ 0.5, 0.5 } ) );
    ASSERT_FALSE( ContainsPoint( outerPolygon, Point2d{ 8.5, 8.5 } ) );
    ...
    const std::size_t lineId =
        segmentSearch.Add( LineSegment2d( Point2d{ 0.0, 0.0 }, Point2d{ 4.0, 0.0 } ) );
    const std::size_t arcId =
        segmentSearch.Add( ArcSegment2d( Point2d{ 4.0, 0.0 }, 1.0, kPi / 2.0, kPi / 2.0 ) );
    ASSERT_TRUE( segmentSearch.Contains( lineId ) );
    ASSERT_TRUE( segmentSearch.Contains( arcId ) );
    ASSERT_TRUE(
        segmentSearch
            .QueryIntersecting( Box2d::FromMinMax( Point2d{ 10.0, 10.0 }, Point2d{ 11.0, 11.0 } ) )
            .empty() );
