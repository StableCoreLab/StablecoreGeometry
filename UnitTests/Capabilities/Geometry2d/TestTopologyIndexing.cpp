...
TEST( TopologyIndexingTest, CoversCurrentCapabilities )
{
    ...
    ASSERT_EQ( boxTree.Query( Box2d::FromMinMax( Point2d{ 1.0, 1.0 }, Point2d{ 3.1, 3.1 } ) ).size(),
               2 );
    ...
    const Polygon2d inner( Polyline2d(
    ...
    const auto duplicateTopology = BuildPolygonTopology( MultiPolygon2d{ outer, outer } );
    ASSERT_EQ( duplicateTopology.Roots().size(), 1 );
    ASSERT_EQ( duplicateTopology.ParentOf( 1 ), 0 );

    const Polygon2d touching( Polyline2d(
    ...
    const Polygon2d intersecting( Polyline2d(
    ...
    const Polygon2d sharedEdgeTouch( Polyline2d(
    ...
    const Polygon2d overlapWithInterior( Polyline2d(
    ...
    const Polygon2d sameOuterWithHole( Polyline2d( { Point2d{ 0.0, 0.0 }, Point2d{ 10.0, 0.0 },
                                                     Point2d{ 10.0, 10.0 }, Point2d{ 0.0, 10.0 } },
                                                   PolylineClosure::Closed ),
                                       { Polyline2d( { Point2d{ 3.0, 3.0 }, Point2d{ 3.0, 7.0 },
                                                       Point2d{ 7.0, 7.0 }, Point2d{ 7.0, 3.0 } },
                                                     PolylineClosure::Closed ) } );
    ...
    const Polygon2d sameOuterWithDifferentHole(
        ...
        { Polyline2d(
            ...
            PolylineClosure::Closed ) } );
    ASSERT_EQ( Relate( sameOuterWithHole, sameOuterWithDifferentHole ),
               PolygonContainment2d::Intersecting );

    const Polygon2d boundaryOverlappingContained( Polyline2d(
        { Point2d{ 2.0, 0.0 }, Point2d{ 6.0, 0.0 }, Point2d{ 6.0, 10.0 }, Point2d{ 2.0, 10.0 } },
        PolylineClosure::Closed ) );
    ASSERT_EQ( Relate( outer, boundaryOverlappingContained ),
               PolygonContainment2d::FirstContainsSecond );

    const Polygon2d noisyOuter(
        Polyline2d( { Point2d{ 0.0, 0.0 }, Point2d{ 10.0, 0.0 }, Point2d{ 10.0, 5.0 },
                      Point2d{ 10.0, 5.000000001 }, Point2d{ 10.0, 10.0 }, Point2d{ 0.0, 10.0 },
                      Point2d{ 0.0, 5.000000001 }, Point2d{ 0.0, 5.0 } },
                    PolylineClosure::Closed ) );
    const Polygon2d smallInner( Polyline2d(
