...
TEST( OffsetTest, CoversCurrentCapabilities )
{
    ...
    const ArcSegment2d ccwArc( Point2d{ 0.0, 0.0 }, 5.0, 0.0, std::acos( -1.0 ) * 0.5 );
    ...
    const Polyline2d openPath( { Point2d{ 0.0, 0.0 }, Point2d{ 4.0, 0.0 }, Point2d{ 4.0, 3.0 } },
    ...
    GEOMETRY_TEST_ASSERT_POINT_NEAR( grownPolygon.OuterRing().PointAt( 0 ), ( Point2d{ -1.0, -1.0 } ),
                                     1e-12 );
    GEOMETRY_TEST_ASSERT_POINT_NEAR( grownPolygon.OuterRing().PointAt( 2 ), ( Point2d{ 5.0, 5.0 } ),
                                     1e-12 );
    ...
    GEOMETRY_TEST_ASSERT_POINT_NEAR( offsetWithHole.OuterRing().PointAt( 0 ),
                                     ( Point2d{ -0.25, -0.25 } ), 1e-12 );
    GEOMETRY_TEST_ASSERT_POINT_NEAR( offsetWithHole.OuterRing().PointAt( 2 ), ( Point2d{ 4.25, 4.25 } ),
                                     1e-12 );

    const Polygon2d concave(
        Polyline2d( { Point2d{ 0.0, 0.0 }, Point2d{ 5.0, 0.0 }, Point2d{ 5.0, 1.0 }, Point2d{ 2.0, 1.0 },
    ...
    const MultiPolygon2d disjoint{ Polygon2d( Polyline2d( { Point2d{ 0.0, 0.0 }, Point2d{ 2.0, 0.0 },
                                                            Point2d{ 2.0, 2.0 }, Point2d{ 0.0, 2.0 } },
                                                          PolylineClosure::Closed ) ),
                                   Polygon2d( Polyline2d( { Point2d{ 4.0, 0.0 }, Point2d{ 6.0, 0.0 },
                                                            Point2d{ 6.0, 2.0 }, Point2d{ 4.0, 2.0 } },
                                                          PolylineClosure::Closed ) ) };
    const MultiPolygon2d expandedDisjoint = Offset( disjoint, 0.5 );
    ASSERT_EQ( expandedDisjoint.Count(), 2 );

    const Polygon2d clockwiseOuter( Polyline2d(
    ...
    const Polygon2d narrowDonut( Polyline2d( { Point2d{ 0.0, 0.0 }, Point2d{ 10.0, 0.0 },
                                               Point2d{ 10.0, 6.0 }, Point2d{ 0.0, 6.0 } },
                                             PolylineClosure::Closed ),
                                 { Polyline2d( { Point2d{ 3.5, 2.0 }, Point2d{ 3.5, 4.0 },
                                                 Point2d{ 6.5, 4.0 }, Point2d{ 6.5, 2.0 } },
                                               PolylineClosure::Closed ) } );
    ...
    const MultiPolygon2d multiWithHole{ narrowDonut, Polygon2d( Polyline2d(
                                                         { Point2d{ 14.0, 0.0 }, Point2d{ 20.0, 0.0 },
                                                           Point2d{ 20.0, 5.0 }, Point2d{ 14.0, 5.0 } },
                                                         PolylineClosure::Closed ) ) };
    ...
    for( std::size_t polygonIndex = 0; polygonIndex < inwardMultiRecovered.Count(); ++polygonIndex )
    {
        ...
        for( std::size_t pointIndex = 0; pointIndex < recovered.OuterRing().PointCount(); ++pointIndex )
        {
            ...
            const bool inSecond = LocatePoint( point, multiWithHole[1] ) != PointContainment2d::Outside;
...
{
    const Polygon2d source( Polyline2d( { Point2d{ 0.0, 0.0 }, Point2d{ 4.0, 0.0 }, Point2d{ 4.0, 4.0 },
                                          Point2d{ 0.0, 4.0 } },
                                        PolylineClosure::Closed ),
                            { Polyline2d( { Point2d{ 1.0, 1.0 }, Point2d{ 1.0, 3.0 },
...
{
    const Polygon2d source( Polyline2d(
        { Point2d{ 0.0, 0.0 }, Point2d{ 6.0, 0.0 }, Point2d{ 6.0, 1.0 }, Point2d{ 2.5, 1.0 },
          Point2d{ 2.5, 4.0 }, Point2d{ 6.0, 4.0 }, Point2d{ 6.0, 5.0 }, Point2d{ 0.0, 5.0 } },
        PolylineClosure::Closed ) );
...
{
    const Polygon2d source( Polyline2d(
        { Point2d{ 0.0, 0.0 }, Point2d{ 2.0, 0.0 }, Point2d{ 2.0, 0.8 }, Point2d{ 4.0, 0.8 },
          Point2d{ 4.0, 0.0 }, Point2d{ 6.0, 0.0 }, Point2d{ 6.0, 2.0 }, Point2d{ 4.0, 2.0 },
          Point2d{ 4.0, 1.2 }, Point2d{ 2.0, 1.2 }, Point2d{ 2.0, 2.0 }, Point2d{ 0.0, 2.0 } },
        PolylineClosure::Closed ) );
    ...
    for( std::size_t polygonIndex = 0; polygonIndex < split.Count(); ++polygonIndex )
    {
        ...
        {
            ASSERT_NE( LocatePoint( piece.OuterRing().PointAt( pointIndex ), source ),
