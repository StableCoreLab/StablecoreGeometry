...
TEST( SearchPolyCapabilityTest, SearchPolygonsBuildsRepresentativeClosedCandidate )
{
    ...
    EXPECT_EQ( result.candidates.front().dominantSyntheticEdgeKind,
               SearchPolySyntheticEdgeKind2d::Unknown );
    EXPECT_EQ( result.candidates.front().dominantSyntheticEdgeSource,
...
TEST( SearchPolyCapabilityTest, SearchPolygonsReportsRepairDiagnosticsForNearClosedLoop )
{
    ...
    EXPECT_EQ( result.candidates.front().dominantPenaltyKind,
               SearchPolyPenaltyKind2d::SyntheticClosure );
    EXPECT_EQ( result.candidates.front().dominantSyntheticEdgeKind,
               SearchPolySyntheticEdgeKind2d::GapClosure );
    EXPECT_EQ( result.candidates.front().dominantSyntheticEdgeSource,
               SearchPolySyntheticEdgeSource2d::SingleGapClose );
    EXPECT_EQ( result.bestCandidateSyntheticEdgeKind, SearchPolySyntheticEdgeKind2d::GapClosure );
    EXPECT_EQ( result.bestCandidateSyntheticEdgeSource,
               SearchPolySyntheticEdgeSource2d::SingleGapClose );
...
TEST( SearchPolyCapabilityTest, SearchPolygonsRanksCleanCandidateAheadOfSyntheticPeer )
{
    ...
    ASSERT_EQ( second.inferredSyntheticEdgeStartVertexIndices.size(),
               second.inferredSyntheticEdgeCount );
    ...
    ASSERT_EQ( second.inferredSyntheticEdgeDanglingTouchCounts.size(),
               second.inferredSyntheticEdgeCount );
...
TEST( SearchPolyCapabilityTest, SearchPolygonsReportsFakeEdgeDiagnosticsForAmbiguousCandidate )
{
    ...
    EXPECT_LT( result.candidates.front().branchScore, result.candidates.front().absoluteArea );
    EXPECT_EQ( result.bestCandidateSyntheticEdgeCount,
               result.candidates.front().inferredSyntheticEdgeCount );
    EXPECT_DOUBLE_EQ( result.bestCandidateSyntheticPerimeter,
                      result.candidates.front().inferredSyntheticPerimeter );
    EXPECT_EQ( result.bestCandidateSyntheticEdgeKind, SearchPolySyntheticEdgeKind2d::GapClosure );
    EXPECT_EQ( result.bestCandidateSyntheticEdgeSource,
               SearchPolySyntheticEdgeSource2d::SingleGapClose );
    EXPECT_DOUBLE_EQ( result.runnerUpSyntheticPerimeter, 0.0 );
...
TEST( SearchPolyCapabilityTest, SearchPolygonsClassifiesBranchCleanupSyntheticEdges )
{
    ...
    ASSERT_EQ( candidate.inferredSyntheticEdgeStartVertexIndices.size(),
               candidate.inferredSyntheticEdgeCount );
    ASSERT_EQ( candidate.inferredSyntheticEdgeEndVertexIndices.size(),
               candidate.inferredSyntheticEdgeCount );
    ASSERT_EQ( candidate.inferredSyntheticEdgeStartDegrees.size(),
               candidate.inferredSyntheticEdgeCount );
    ASSERT_EQ( candidate.inferredSyntheticEdgeEndDegrees.size(), candidate.inferredSyntheticEdgeCount );
    ASSERT_EQ( candidate.inferredSyntheticEdgeDanglingTouchCounts.size(),
               candidate.inferredSyntheticEdgeCount );
    ASSERT_EQ( candidate.inferredSyntheticEdgeBranchTouchCounts.size(),
               candidate.inferredSyntheticEdgeCount );
    ...
    {
        EXPECT_LT( candidate.inferredSyntheticEdgeStartVertexIndices[index],
                   result.diagnostics.uniqueVertexCount );
        EXPECT_LT( candidate.inferredSyntheticEdgeEndVertexIndices[index],
                   result.diagnostics.uniqueVertexCount );
...
TEST( SearchPolyCapabilityTest, SearchPolygonContainingPointPreservesSyntheticExplanation )
{
    ...
    for( std::size_t index = 0; index < candidate->inferredSyntheticEdgeCount; ++index )
    {
        EXPECT_EQ( candidate->inferredSyntheticEdgeKinds[index],
                   SearchPolySyntheticEdgeKind2d::GapClosure );
        EXPECT_EQ( candidate->inferredSyntheticEdgeSources[index],
                   SearchPolySyntheticEdgeSource2d::SingleGapClose );
