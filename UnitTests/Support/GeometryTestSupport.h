...
namespace Geometry::Test
{
    namespace Detail
    {
        ...
        [[noreturn]] inline void Fail( std::string_view kind, std::string_view file, int line,
    ...
    inline void AssertPointNear( const Actual &actual, const Expected &expected, double eps,
                                 std::string_view actualExpr, std::string_view expectedExpr,
                                 std::string_view file, int line )
    {
        ...
        Detail::Fail( "point mismatch", file, line,
    ...
    template <Detail::HasXY Actual, Detail::HasXY Expected>
    inline void AssertVectorNear( const Actual &actual, const Expected &expected, double eps,
                                  std::string_view actualExpr, std::string_view expectedExpr,
                                  std::string_view file, int line )
    {
    ...
    inline void AssertNear( const Actual &actual, const Expected &expected, double eps,
                            std::string_view actualExpr, std::string_view expectedExpr,
                            std::string_view file, int line )
    {
        ...
        Detail::Fail( "value mismatch", file, line,
    ...
    template <typename Actual, typename Expected>
    inline void AssertBoxNear( const Actual &actual, const Expected &expected, double eps,
                               std::string_view actualExpr, std::string_view expectedExpr,
                               std::string_view file, int line )
    {
        if constexpr( Detail::HasIsValid<Actual> && Detail::HasIsValid<Expected> )
        {
            ...
            {
                Detail::Fail( "box validity mismatch", file, line,
                              std::string( actualExpr ) + " = " + Detail::DescribeBoxLike( actual ) +
                                  ", " + std::string( expectedExpr ) + " = " +
                                  Detail::DescribeBoxLike( expected ) );
            }
        ...
        AssertPointNear( Detail::MinPointLike( actual ), Detail::MinPointLike( expected ), eps,
                         actualExpr, expectedExpr, file, line );
        AssertPointNear( Detail::MaxPointLike( actual ), Detail::MaxPointLike( expected ), eps,
                         actualExpr, expectedExpr, file, line );
    }

    template <typename Actual, typename Expected>
    inline void AssertProjectionNear( const Actual &actual, const Expected &expected, double eps,
                                      std::string_view actualExpr, std::string_view expectedExpr,
                                      std::string_view file, int line )
    {
        ...
        AssertNear( actual.distanceSquared, expected.distanceSquared, eps, actualExpr, expectedExpr,
                    file, line );

        if( actual.isOnSegment != expected.isOnSegment )
        {
            Detail::Fail( "projection flag mismatch", file, line,
                          std::string( actualExpr ) + " = " + Detail::DescribeProjectionLike( actual ) +
                              ", " + std::string( expectedExpr ) + " = " +
                              Detail::DescribeProjectionLike( expected ) );
        }
    }

    template <Detail::HasPointCount Actual, Detail::HasPointCount Expected>
    inline void AssertPolylineNear( const Actual &actual, const Expected &expected, double eps,
                                    std::string_view actualExpr, std::string_view expectedExpr,
                                    std::string_view file, int line )
    {
        if( actual.PointCount() != expected.PointCount() )
        {
            Detail::Fail( "polyline point-count mismatch", file, line,
                          std::string( actualExpr ) + " = " + Detail::DescribePolylineLike( actual ) +
                              ", " + std::string( expectedExpr ) + " = " +
                              Detail::DescribePolylineLike( expected ) );
        }
                      ...
                      } )
        {
            ...
            {
                Detail::Fail(
                    "polyline closure mismatch", file, line,
                    std::string( actualExpr ) + " = " + Detail::DescribePolylineLike( actual ) + ", " +
        ...
        {
            AssertPointNear( actual.PointAt( i ), expected.PointAt( i ), eps, actualExpr, expectedExpr,
                             file, line );
        }
    }

    template <typename Actual, typename Expected>
    inline void AssertPolygonNear( const Actual &actual, const Expected &expected, double eps,
                                   std::string_view actualExpr, std::string_view expectedExpr,
                                   std::string_view file, int line )
    {
        if( actual.HoleCount() != expected.HoleCount() )
        {
            Detail::Fail( "polygon hole-count mismatch", file, line,
                          std::string( actualExpr ) + " = " + Detail::DescribePolygonLike( actual ) +
                              ", " + std::string( expectedExpr ) + " = " +
                              Detail::DescribePolygonLike( expected ) );
        }

        AssertPolylineNear( actual.OuterRing(), expected.OuterRing(), eps, actualExpr, expectedExpr,
                            file, line );
        for( std::size_t i = 0; i < actual.HoleCount(); ++i )
        {
            AssertPolylineNear( actual.HoleAt( i ), expected.HoleAt( i ), eps, actualExpr, expectedExpr,
                                file, line );
        }
    }
    ...
    [[nodiscard]] inline Geometry::Polyline2d MakeRectangleRing( const Geometry::Point2d &minPoint,
    ...
    [[nodiscard]] inline Geometry::Polyline2d MakeRectangleHoleRing( const Geometry::Point2d &minPoint,
                                                                     const Geometry::Point2d &maxPoint )
    {
        return MakePolyline(
            {
    ...
    [[nodiscard]] inline Geometry::Polygon2d MakeRectanglePolygon( const Geometry::Point2d &minPoint,
                                                                   const Geometry::Point2d &maxPoint )
    {
...
#define GEOMETRY_TEST_ASSERT_NEAR( actual, expected, eps ) \
    ::Geometry::Test::AssertNear( ( actual ), ( expected ), ( eps ), #actual, #expected, __FILE__, \
                                  __LINE__ )

#define GEOMETRY_TEST_ASSERT_POINT_NEAR( actual, expected, eps ) \
    ::Geometry::Test::AssertPointNear( ( actual ), ( expected ), ( eps ), #actual, #expected, __FILE__, \
                                       __LINE__ )

#define GEOMETRY_TEST_ASSERT_VECTOR_NEAR( actual, expected, eps ) \
    ::Geometry::Test::AssertVectorNear( ( actual ), ( expected ), ( eps ), #actual, #expected, \
                                        __FILE__, __LINE__ )

#define GEOMETRY_TEST_ASSERT_BOX_NEAR( actual, expected, eps ) \
    ::Geometry::Test::AssertBoxNear( ( actual ), ( expected ), ( eps ), #actual, #expected, __FILE__, \
                                     __LINE__ )

#define GEOMETRY_TEST_ASSERT_PROJECTION_NEAR( actual, expected, eps ) \
    ::Geometry::Test::AssertProjectionNear( ( actual ), ( expected ), ( eps ), #actual, #expected, \
                                            __FILE__, __LINE__ )

#define GEOMETRY_TEST_ASSERT_POLYLINE_NEAR( actual, expected, eps ) \
    ::Geometry::Test::AssertPolylineNear( ( actual ), ( expected ), ( eps ), #actual, #expected, \
                                          __FILE__, __LINE__ )

#define GEOMETRY_TEST_ASSERT_POLYGON_NEAR( actual, expected, eps ) \
    ::Geometry::Test::AssertPolygonNear( ( actual ), ( expected ), ( eps ), #actual, #expected, \
                                         __FILE__, __LINE__ )
