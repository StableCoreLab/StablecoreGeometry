...
TEST( Brep3dGapTest, CoedgeLoopOwnershipEditingWorkflowRemainsOpen )
{
    ...
    (void)v1;
    (void)edge;
    GTEST_SKIP() << "Known 3D gap: Brep coedge-loop ownership/shell-consistency editing workflow beyond "
                    "currently covered replacement subsets (single-face rebuilt body + multi-face "
...
TEST( Brep3dGapTest, NonPlanarTrimmedFaceTopologyRepairRemainsOpen )
{
    ...
    (void)body;
    GTEST_SKIP()
        << "Known 3D gap: non-planar trimmed face topology repair and robust shell stitching are still "
           "open. "
           "Covered subsets: single planar face missing-trim conservative backfill (horizontal z=0 "
           "plane), "
           "single planar face missing-trim backfill with non-horizontal (y=0 vertical plane, +y "
           "normal), "
