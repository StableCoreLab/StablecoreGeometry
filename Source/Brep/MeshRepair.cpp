...
namespace Geometry
{
    namespace
    {
        ...
        [[nodiscard]] PlaneBasis BuildPlaneBasis( const std::vector<Point3d> &loopVertices, double eps )
        {
            PlaneBasis basis;
        ...
        [[nodiscard]] bool IsPlanarLoop( const std::vector<Point3d> &loopVertices,
                                         const PlaneBasis &basis, double eps )
        {
        ...
        [[nodiscard]] Point2d ProjectToPlane( const Point3d &point, const PlaneBasis &basis )
        {
            ...
            while( !queue.empty() )
            {
                ...
                for( std::size_t neighbor : adjacency[triangleIndex].adjacentTriangles )
                {
                    ...
                    const bool sameDirection =
                        HasSameSharedEdgeDirection( mesh, triangleIndex, neighbor );
    ...
    TriangleMeshRepair3d ClosePlanarBoundaryLoops( const TriangleMesh &mesh, double eps )
    {
        ...
        for( const MeshBoundaryLoop3d &loop : boundaryLoops )
        {
            ...
            {
                triangles.push_back( TriangleMesh::TriangleIndices{ loop.vertexIndices[tri[0]],
