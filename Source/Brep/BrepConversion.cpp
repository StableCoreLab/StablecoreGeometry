...
namespace Geometry
{
    namespace
    {
        ...
        [[nodiscard]] bool BuildFaceWithRefitSupportPlane(
            const PolyhedronFace3d &face, PolyhedronFace3d &repairedFace, double eps,
        ...
        [[nodiscard]] std::size_t FindOrAddRepresentativePoint(
            const Point3d &point, std::vector<Point3d> &representativePoints, double eps )
        {
                    ...
                    for( std::size_t i = 0; i < loop.VertexCount(); ++i )
                    {
                        for( std::size_t j = i + 1; j < loop.VertexCount(); ++j )
                        {
                            const double distanceSquared =
                                ( loop.VertexAt( i ) - loop.VertexAt( j ) ).LengthSquared();
        ...
        void BuildBodyLoopRepresentativeIds( const PolyhedronBody &body,
                                             std::vector<FaceLoopRepresentativeIds> &ids, double eps )
        {
            ids.clear();
            ...
            const Plane referencePlane =
                body.FaceCount() > 0 ? body.FaceAt( 0 ).SupportPlane() : Plane{};
            ...
            if( useCommonSupportPlaneGrouping )
            {
                for( std::size_t faceIndex = 1; faceIndex < body.FaceCount(); ++faceIndex )
                {
                    const Vector3d faceNormal =
                        body.FaceAt( faceIndex ).SupportPlane().UnitNormal( eps );
            ...
            for( std::size_t faceIndex = 0; faceIndex < body.FaceCount(); ++faceIndex )
            {
                ...
                {
                    faceIds.outer.push_back( FindOrAddRepresentativePoint(
                        groupPoint( face.OuterLoop().VertexAt( i ) ), representativePoints,
                ...
                for( std::size_t holeIndex = 0; holeIndex < face.HoleCount(); ++holeIndex )
                {
                    ...
                    {
                        holeIds.push_back( FindOrAddRepresentativePoint(
                            groupPoint( hole.VertexAt( i ) ), representativePoints,
                            Geometry::kRepresentativeGroupingEpsilon ) );
                    }
                }
            }
        ...
        [[nodiscard]] bool ComputeRepresentativeTargetPointsForFaceRange(
            std::size_t faceCount, const FaceAccessor &faceAt,
            const std::vector<FaceLoopRepresentativeIds> &representativeIds,
            std::unordered_map<std::size_t, Point3d> &representativeTargetPoints, double eps )
        {
            ...
            auto accumulateLoop = [&]( const PolyhedronLoop3d &loop,
                ...
                for( std::size_t i = 0; i < loop.VertexCount(); ++i )
                {
                    ...
                    if( hasPrevious && representativeId == previousId &&
                        point.AlmostEquals( previousPoint, eps ) )
                    {
                        continue;
                    }
            ...
            for( const auto &[id, accumulator] : accumulators )
            {
                ...
                if( accumulator.hasPreferredPlane )
                {
                    ...
                    const double signedDistance =
                        accumulator.preferredPlane.SignedDistanceTo( target, eps );
            ...
            std::unordered_map<std::size_t, Point3d> &representativeTargetPoints )
        {
            return ComputeRepresentativeTargetPointsForFaceRange(
                faces.size(),
                [&faces]( const std::size_t faceIndex ) -> const PolyhedronFace3d & {
                    return faces[faceIndex];
                },
                representativeIds, representativeTargetPoints, Geometry::kRepresentativeMatchEpsilon );
        }

        [[nodiscard]] bool ExecuteRepresentativeTargetAggregationPass(
            const PolyhedronBody &body, const std::vector<FaceLoopRepresentativeIds> &representativeIds,
            std::unordered_map<std::size_t, Point3d> &representativeTargetPoints )
        {
            return ComputeRepresentativeTargetPointsForFaceRange(
                body.FaceCount(),
        ...
        [[nodiscard]] bool ExecuteCrossFaceSnappingPass(
            ...
            std::vector<PolyhedronFace3d> &faces, double eps )
        {
            ...
            auto snapLoopToTargets = [&]( const PolyhedronLoop3d &loop,
                                          const std::vector<std::size_t> &loopIds ) {
                ...
                for( std::size_t i = 0; i < loop.VertexCount(); ++i )
                {
                    ...
                    const Point3d target = targetIt != representativeTargetPoints.end()
                                               ? targetIt->second
                                               : loop.VertexAt( i );
            ...
            for( std::size_t faceIndex = 0; faceIndex < faces.size(); ++faceIndex )
            {
                ...
                for( std::size_t holeIndex = 0; holeIndex < face.HoleCount(); ++holeIndex )
                {
                    PolyhedronLoop3d hole =
                        snapLoopToTargets( face.HoleAt( holeIndex ), ids.holes[holeIndex] );
                    if( !hole.IsValid( eps ) )
                    {
                        return false;
                    }
                ...
                PolyhedronFace3d snappedInput( face.SupportPlane(), std::move( outer ),
                                               std::move( holes ) );
                ...
                if( !BuildFaceWithRefitSupportPlane( snappedInput, snappedFace, eps,
                                                     emptyPreferredOuterVertices, &ids.outer, &ids.holes,
                                                     nullptr ) )
                {
                    return false;
                }
        ...
        [[nodiscard]] bool AppendLoopVerticesFromBody( const BrepBody &body, const BrepLoop &loop,
                                                       std::vector<Point3d> &vertices, double eps )
        {
            ...
            for( std::size_t i = 0; i < loop.CoedgeCount(); ++i )
            {
                ...
                if( vertices.empty() ||
                    !vertices.back().AlmostEquals( point, Geometry::kLoopCleanupEpsilon ) )
                {
        ...
        [[nodiscard]] bool AppendLoopVerticesFromBrepTopology( const std::vector<BrepVertex> &vertices,
        ...
        [[nodiscard]] bool BuildLoopFromTrim( const CurveOnSurface &trim, std::vector<Point3d> &vertices,
                                              double eps )
        {
        ...
        [[nodiscard]] bool BuildPolyhedronFaceFromBodyFace( const BrepBody &body, const BrepFace &face,
                                                            PolyhedronFace3d &polyFace, double eps )
        {
            ...
            polyFace =
                PolyhedronFace3d( planeSurface->SupportPlane(),
                                  PolyhedronLoop3d( std::move( outerVertices ) ), std::move( holes ) );
        ...
        [[nodiscard]] bool AppendBrepLoopFromPolyLoop( const PolyhedronLoop3d &polyLoop,
                                                       ...
                                                       std::vector<BrepEdge> &edges, BrepLoop &loop,
        ...
        [[nodiscard]] std::size_t FindOrAddBrepVertex( const Point3d &point,
                                                       std::vector<BrepVertex> &vertices, double eps,
                                                       const Point3d *preferredPoint = nullptr )
        {
            for( std::size_t i = 0; i < vertices.size(); ++i )
            {
                ...
                {
                    if( preferredPoint != nullptr &&
                        !vertices[i].Point().AlmostEquals( *preferredPoint, eps ) )
                    {
        ...
        [[nodiscard]] bool FindReusableBrepEdge( const std::vector<BrepEdge> &edges,
                                                 std::size_t startVertexIndex,
                                                 std::size_t endVertexIndex, std::size_t &edgeIndex,
                                                 bool &reversed )
        {
            for( std::size_t i = 0; i < edges.size(); ++i )
            {
                const BrepEdge &edge = edges[i];
                if( edge.StartVertexIndex() == startVertexIndex &&
                ...
                if( edge.StartVertexIndex() == endVertexIndex &&
                    edge.EndVertexIndex() == startVertexIndex )
                {
                    edgeIndex = i;
                    reversed = true;
                    return true;
        ...
        [[nodiscard]] bool AppendSharedBrepLoopFromPolyLoop(
            const PolyhedronLoop3d &polyLoop, std::vector<BrepVertex> &vertices,
            std::vector<BrepEdge> &edges, BrepLoop &loop, std::vector<Point2d> &uvPoints,
            const std::vector<std::size_t> *representativeIds,
            const std::unordered_map<std::size_t, Point3d> *representativeTargetPoints,
            const Plane *representativeTargetPlane, bool useAxisAlignedRepresentativeSnapping,
            std::unordered_map<std::size_t, std::size_t> *representativeToVertexIndex,
            double vertexDedupEps, double eps )
        {
            try
            {
                ...
                auto snapToRepresentativePlaneAxis = [&]( Point3d point ) {
                    ...
                    const Vector3d absNormal{ std::abs( unitNormal.x ), std::abs( unitNormal.y ),
                ...
                for( std::size_t i = 0; i < vertexCount; ++i )
                {
                    ...
                    if( hasRepresentativeIds )
                    {
                        ...
                        if( found != representativeToVertexIndex->end() )
                        {
                            ...
                            {
                                const auto representativePointIt =
                                    representativeTargetPoints->find( representativeId );
                                if( representativePointIt != representativeTargetPoints->end() &&
                                    !vertices[found->second].Point().AlmostEquals(
                                        representativePointIt->second, vertexDedupEps ) )
                                {
                                    vertices[found->second] = BrepVertex(
                                        snapToRepresentativePlaneAxis( representativePointIt->second ) );
                                }
                            }
                    ...
                    {
                        loopVertexIndices.push_back(
                            FindOrAddBrepVertex( point, vertices, vertexDedupEps ) );
                    }
                }
                ...
                for( std::size_t i = 0; i < vertexCount; ++i )
                {
                    ...
                    bool reversed = false;
                    if( !FindReusableBrepEdge( edges, startVertexIndex, endVertexIndex, edgeIndex,
                                               reversed ) )
                    {
        ...
        void BuildSharedOuterVertexPreferenceMask(
            const PolyhedronBody &body, std::vector<std::vector<bool>> &facePreferredOuterVertices,
            double eps )
        {
            ...
            FaceLoopRepresentativeIds *repairedRepresentativeIds )
        {
            ...
            auto normalizeLoop = [&]( const PolyhedronLoop3d &loop, PolyhedronLoop3d &normalized,
                ...
                while( vertices.size() >= 2 &&
                       vertices.front().AlmostEquals( vertices.back(), supportEps ) )
                {
                    vertices.pop_back();
                ...
                sourceOuterRepresentativeIds->size() == face.OuterLoop().VertexCount() )
            {
                ...
                {
                    normalizedOuterRepresentativeIds.push_back(
                        ( *sourceOuterRepresentativeIds )[sourceIndex] );
                }
            }
            ...
            candidatePoints.reserve(
                outer.VertexCount() +
                std::accumulate( holes.begin(), holes.end(), std::size_t{ 0 },
                                 []( std::size_t total, const PolyhedronLoop3d &loop ) {
                                     return total + loop.VertexCount();
                                 } ) );
            for( std::size_t i = 0; i < outer.VertexCount(); ++i )
            {
                candidatePoints.push_back(
                    CandidatePoint{ outer.VertexAt( i ), normalizedPreferredOuter[i] } );
            }
            ...
            auto scorePlane = [&]( const Plane &plane ) {
                ...
                const auto accumulateLoopDistance = [&]( const PolyhedronLoop3d &loop, double weight ) {
                    for( std::size_t vertexIndex = 0; vertexIndex < loop.VertexCount(); ++vertexIndex )
                    {
                        const double distance =
                            std::abs( plane.SignedDistanceTo( loop.VertexAt( vertexIndex ), eps ) );
                ...
                return std::tuple<std::size_t, double, double, double>{ onPlaneCount, holeDistance,
            ...
            if( preferHoleDominatedPlane )
            {
                    ...
                    for( std::size_t i = 0; i + 2 < hole.VertexCount() && !seededHolePlane; ++i )
                    {
                        ...
                        for( std::size_t j = i + 1; j + 1 < hole.VertexCount() && !seededHolePlane; ++j )
                        {
                            ...
                            for( std::size_t k = j + 1; k < hole.VertexCount(); ++k )
                            {
                                ...
                                if( holeDistance > Geometry::kSupportPlaneOnPlaneEpsilon *
                                                       static_cast<double>( hole.VertexCount() ) )
                                {
                                    continue;
                                }
                ...
                if( foundSupport )
                {
                    ...
                    else
                    {
                        ...
                        repairedFace = PolyhedronFace3d( refitPlane, std::move( projectedOuter ),
                                                         std::move( projectedHoles ) );
                        if( !repairedFace.IsValid( faceValidityEps ) )
                        {
                            std::vector<Point3d> reversedOuterVertices =
                                repairedFace.OuterLoop().Vertices();
                            ...
                            for( std::size_t holeIndex = 0; holeIndex < repairedFace.HoleCount();
                                 ++holeIndex )
                            {
                                std::vector<Point3d> reversedHoleVertices =
                                    repairedFace.HoleAt( holeIndex ).Vertices();
                            ...
                            repairedFace = PolyhedronFace3d(
                                refitPlane, PolyhedronLoop3d( std::move( reversedOuterVertices ) ),
                    ...
                    for( std::size_t k = j + 1; k < candidatePoints.size(); ++k )
                    {
                        ...
                        if( !betterFit )
                        {
                            ...
                            else if( preferHoleDominatedPlane )
                            {
                                ...
                                const bool sameHoleDistance =
                                    std::abs( holeDistance - bestHoleDistance ) <= eps;
                                const bool betterTotalDistance = totalDistance + eps < bestTotalDistance;
                                const bool sameTotalDistance =
                                    std::abs( totalDistance - bestTotalDistance ) <= eps;
            ...
            if( holes.empty() )
            {
                ...
                std::reverse( reversedOuterVertices.begin(), reversedOuterVertices.end() );
                repairedFace = PolyhedronFace3d(
                    refitPlane, PolyhedronLoop3d( std::move( reversedOuterVertices ) ), holes );
                if( repairedFace.IsValid( faceValidityEps ) )
                {
                    if( repairedRepresentativeIds != nullptr )
                    {
        ...
        [[nodiscard]] bool ExecuteSupportPlaneScoringPass(
            const PolyhedronBody &body, const std::vector<std::vector<bool>> &facePreferredOuterVertices,
            const std::vector<FaceLoopRepresentativeIds> *sourceRepresentativeIds,
            std::vector<PolyhedronFace3d> &repairedFaces,
            std::vector<FaceLoopRepresentativeIds> &repairedRepresentativeIds, double eps )
        {
            repairedFaces.clear();
            ...
            {
                const std::vector<bool> &preferredOuterVertices =
                    faceIndex < facePreferredOuterVertices.size() ? facePreferredOuterVertices[faceIndex]
                ...
                if( !BuildFaceWithRefitSupportPlane(
                        body.FaceAt( faceIndex ), repairedFace, eps, preferredOuterVertices,
                        sourceOuterIds, sourceHoleIds,
                        sourceRepresentativeIds != nullptr ? &repairedIds : nullptr ) )
                {
                    if( !BuildFaceWithRefitSupportPlane( body.FaceAt( faceIndex ), repairedFace, eps,
                                                         emptyPreferredOuterVertices, nullptr, nullptr,
                                                         nullptr ) )
                    {
                        return false;
                    }
                    ...
                    if( sourceRepresentativeIds != nullptr &&
                        faceIndex < sourceRepresentativeIds->size() )
                    {
        ...
        [[nodiscard]] bool ExecuteTopologyReconciliationPass(
            const std::vector<PolyhedronFace3d> &faces,
            const std::vector<FaceLoopRepresentativeIds> &representativeIds,
            NonPlanarRepairPassResult &repairResult, double eps )
        {
            ...
            if( !representativeIds.empty() && !ExecuteRepresentativeTargetAggregationPass(
                                                  repairResult.body, repairResult.representativeIds,
            ...
            NonPlanarRepairPassResult &repairResult, double eps )
        {
            ...
            if( !ExecuteSupportPlaneScoringPass( body, facePreferredOuterVertices,
                                                 sourceRepresentativeIds, repairedFaces,
                                                 repairedRepresentativeIds, eps ) )
            {
                return false;
            }
            ...
            if( !ExecuteTopologyReconciliationPass( repairedFaces, repairedRepresentativeIds,
                                                    repairResult, eps ) )
            {
            ...
            for( int iteration = 0; iteration < 2; ++iteration )
            {
                ...
                std::unordered_map<std::size_t, Point3d> representativeTargetPoints;
                if( !ExecuteRepresentativeTargetAggregationPass( currentFaces, repairedRepresentativeIds,
                ...
                if( !ExecuteCrossFaceSnappingPass( repairedRepresentativeIds, representativeTargetPoints,
                                                   snappedFaces, eps ) )
                {
                    break;
                }
                ...
                if( !ExecuteTopologyReconciliationPass( snappedFaces, repairedRepresentativeIds,
                                                        snappedResult, eps ) )
                {
                    break;
                }
    ...
    BrepFaceConversion3d ConvertToPolyhedronFace( const BrepFace &face, double eps )
    {
        ...
        PolyhedronFace3d polyFace( planeSurface->SupportPlane(),
                                   PolyhedronLoop3d( std::move( outerVertices ) ), std::move( holes ) );
    ...
    PolyhedronBrepBodyConversion3d ConvertToBrepBody( const PolyhedronBody &body, double eps )
    {
        try
        {
            auto hasCommonSupportNormal = [&]( const PolyhedronBody &candidateBody ) {
                ...
                {
                    const Vector3d faceNormal =
                        candidateBody.FaceAt( faceIndex ).SupportPlane().UnitNormal( eps );
                    if( !faceNormal.IsValid() || ( faceNormal - referenceNormal ).Length() > eps )
                    {
                        return false;
                    }
            ...
            const Plane commonSupportPlane =
                sourceBody.FaceCount() > 0 ? sourceBody.FaceAt( 0 ).SupportPlane() : Plane{};
            const bool requiresRepair = !sourceBody.IsValid( eps );
            if( requiresRepair && !TryRepairPolyhedronBodyForBrepConversion(
                                      body, &sourceRepresentativeIds, repairResult, eps ) )
            {
                return { false, BrepConversionIssue3d::InvalidBody, 0, {} };
            }
                ...
                repairResult.body.FaceCount() == originalBody.FaceCount() )
            {
                ...
                auto accumulateProjectedLoop = [&]( const PolyhedronLoop3d &loop,
                    ...
                    for( std::size_t i = 0; i < loop.VertexCount(); ++i )
                    {
                        ...
                        const std::size_t clusterIndex =
                            FindOrAddRepresentativePoint( point, projectedClusterRepresentatives,
                                                          Geometry::kRepresentativeGroupingEpsilon );
                ...
                for( std::size_t faceIndex = 0; faceIndex < originalBody.FaceCount(); ++faceIndex )
                {
                    ...
                    for( std::size_t holeIndex = 0;
                         ...
                         ++holeIndex )
                    {
                        (void)accumulateProjectedLoop( originalFace.HoleAt( holeIndex ),
                                                       ids.holes[holeIndex], supportPlane );
                    }
                }
                ...
                for( const auto &[representativeId, clusterIndex] : repairIdToProjectedCluster )
                {
                    ...
                    const RepresentativePointAccumulator &accumulator =
                        projectedClusterAccumulators[clusterIndex];
                    if( accumulator.count == 0 )
                    {
                        continue;
                    }
                    ...
                    const Vector3d average = accumulator.sum / static_cast<double>( accumulator.count );
                    representativeTargetPoints.try_emplace( representativeId,
                                                            Point3d{ average.x, average.y, average.z } );
                }
            }

            if( requiresRepair && !repairResult.representativeIds.empty() &&
                !sourceRepresentativeIds.empty() )
            {
                ...
                for( std::size_t faceIndex = 0; faceIndex < faceCount; ++faceIndex )
                {
                    ...
                    {
                        for( std::size_t vertexIndex = 0; vertexIndex < sourceFaceIds.outer.size();
                             ++vertexIndex )
                        {
                            ...
                            {
                                representativeTargetPoints.try_emplace( repairId,
                                                                        sourceTargetIt->second );
                            }
                        }
                    }

                    const std::size_t holeCount =
                        std::min( sourceFaceIds.holes.size(), repairFaceIds.holes.size() );
                    for( std::size_t holeIndex = 0; holeIndex < holeCount; ++holeIndex )
                    {
                        ...
                        for( std::size_t vertexIndex = 0; vertexIndex < sourceHoleIds.size();
                             ++vertexIndex )
                        {
            ...
            {
                for( const auto &[representativeId, targetPoint] :
                     repairResult.representativeTargetPoints )
                {
            ...
            if( useCommonSupportPlaneGrouping && commonSupportPlane.IsValid( eps ) )
            {
                ...
                {
                    const double signedDistance =
                        commonSupportPlane.SignedDistanceTo( targetPoint, eps );
                    targetPoint = targetPoint - unitNormal * signedDistance;
                }
            }

            const double vertexDedupEps =
                repairQuadHolelessBody
            ...
            auto buildLoopFromRepresentativeTargets =
                [&representativeTargetPoints]( const PolyhedronLoop3d &loop,
                                               const std::vector<std::size_t> *loopRepresentativeIds ) {
                    if( loopRepresentativeIds == nullptr ||
                        loopRepresentativeIds->size() != loop.VertexCount() )
                    {
                        return loop;
                    }
                    ...
                    {
                        const auto targetIt =
                            representativeTargetPoints.find( ( *loopRepresentativeIds )[i] );
                        points.push_back( targetIt != representativeTargetPoints.end()
                                              ? targetIt->second
                                              : loop.VertexAt( i ) );
                    }
            ...
            for( std::size_t faceIndex = 0; faceIndex < sourceBody.FaceCount(); ++faceIndex )
            {
                ...
                if( faceIndex < repairResult.representativeIds.size() )
                {
                    for( std::size_t holeIndex = 0; holeIndex < sourceFace.HoleCount(); ++holeIndex )
                    {
                        ...
                        {
                            const auto &candidateHoleIds =
                                repairResult.representativeIds[faceIndex].holes[holeIndex];
                ...
                if( faceIndex < sourceRepresentativeIds.size() )
                {
                    for( std::size_t holeIndex = 0; holeIndex < sourceFace.HoleCount(); ++holeIndex )
                    {
                        ...
                        {
                            const auto &candidateHoleIds =
                                sourceRepresentativeIds[faceIndex].holes[holeIndex];
                            if( candidateHoleIds.size() == sourceFace.HoleAt( holeIndex ).VertexCount() )
                            {
                                holeRepresentativeIds[holeIndex] = &candidateHoleIds;
                            }
                ...
                const PlaneSurface planeSurface = PlaneSurface::FromPlane(
                    face.SupportPlane(), Intervald{ -1.0, 1.0 }, Intervald{ -1.0, 1.0 }, validationEps );
                ...
                if( !AppendSharedBrepLoopFromPolyLoop(
                        face.OuterLoop(), vertices, edges, outerLoop, outerUv, outerRepresentativeIds,
                        hasRepresentativeTargetPoints ? &representativeTargetPoints : nullptr,
                        hasRepresentativeTargetPoints ? &supportPlane : nullptr,
                        useCommonSupportPlaneGrouping, &representativeToVertexIndex, vertexDedupEps,
                        eps ) )
                {
                    return { false, BrepConversionIssue3d::InvalidFace, faceIndex, {} };
                }
                ...
                try
                {
                    ...
                    if( !AppendLoopVerticesFromBrepTopology( vertices, edges, outerLoop,
                                                             outerLoopVertices ) )
                    {
                        return { false, BrepConversionIssue3d::InvalidFace, faceIndex, {} };
                    }
                ...
                try
                {
                    outerTrim = CurveOnSurface(
                        surface, Polyline2d( std::move( outerUv ), PolylineClosure::Closed ) );
                    if( !outerTrim.IsValid(
                            GeometryTolerance3d{ validationEps, validationEps, validationEps } ) )
                    {
                        outerTrim = {};
                    }
                }
                ...
                for( std::size_t holeIndex = 0; holeIndex < face.HoleCount(); ++holeIndex )
                {
                    try
                    {
                        ...
                        const std::vector<std::size_t> *holeRepresentativeIdsForFace =
                            holeRepresentativeIds[holeIndex];

                        BrepLoop holeLoop;
                        std::vector<Point2d> holeUv;
                        if( !AppendSharedBrepLoopFromPolyLoop(
                                hole, vertices, edges, holeLoop, holeUv, holeRepresentativeIdsForFace,
                                hasRepresentativeTargetPoints ? &representativeTargetPoints : nullptr,
                                hasRepresentativeTargetPoints ? &supportPlane : nullptr,
                                useCommonSupportPlaneGrouping, &representativeToVertexIndex,
                                vertexDedupEps, eps ) )
                        {
                            return { false, BrepConversionIssue3d::InvalidFace, faceIndex, {} };
                        }
                        ...
                        try
                        {
                            ...
                            if( !AppendLoopVerticesFromBrepTopology( vertices, edges, holeLoop,
                                                                     holeLoopVertices ) )
                            {
                                return { false, BrepConversionIssue3d::InvalidFace, faceIndex, {} };
                            }
                        ...
                        CurveOnSurface holeTrim(
                            surface, Polyline2d( std::move( holeUv ), PolylineClosure::Closed ) );
                        if( !holeTrim.IsValid(
                                GeometryTolerance3d{ validationEps, validationEps, validationEps } ) )
                        {
                            holeTrim = {};
                ...
                try
                {
                    BrepFace brepFace( surface, outerLoop, std::move( holeLoops ),
            ...
            try
            {
                ...
                bool shellClosed = ComputeSharedShellClosed( sourceBody, sourceRepresentativeIds ) ||
                ...
                BrepBody brepBody( std::move( vertices ), std::move( edges ),
                                   { BrepShell( std::move( faces ), shellClosed ) } );
                if( !brepBody.IsValid(
                        GeometryTolerance3d{ validationEps, validationEps, validationEps } ) )
                {
                    return { false, BrepConversionIssue3d::InvalidBody, 0, {} };
