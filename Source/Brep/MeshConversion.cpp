...
namespace Geometry
{
    namespace
    {
        [[nodiscard]] bool AppendLoopVerticesFromBrepLoop( const BrepBody &body, const BrepLoop &loop,
        ...
        [[nodiscard]] bool BuildPolyhedronFaceFromBrepFace( const BrepBody &body, const BrepFace &face,
        ...
        [[nodiscard]] bool BuildPolygonFromBrepFaceTrims( const BrepFace &face, Polygon2d &polygon )
        {
        ...
        [[nodiscard]] Point3d LiftToSurface( const Point2d &uv, const Surface &surface )
        {
            return surface.PointAt( uv.x, uv.y );
        }

        [[nodiscard]] std::size_t FindOrAddMeshVertex( const Point3d &point,
                                                       std::vector<Point3d> &vertices, double eps )
        {
        ...
        [[nodiscard]] bool PointInTriangle2d( const Point2d &point, const Point2d &a, const Point2d &b,
                                              const Point2d &c, double eps )
        {
        ...
        [[nodiscard]] bool IsEar( const std::vector<std::size_t> &polygon, std::size_t index,
                                  const std::vector<Point2d> &projected, double orientationSign,
                                  double eps )
        {
        ...
        [[nodiscard]] bool TriangulateSimplePolygon(
            const std::vector<Point2d> &projected, std::vector<TriangleMesh::TriangleIndices> &triangles,
            double eps )
        {
            if( projected.size() < 3 )
            {
        ...
        [[nodiscard]] std::vector<Point2d> RemoveAdjacentDuplicates( const std::vector<Point2d> &points,
                                                                     double eps )
        {
        ...
        [[nodiscard]] std::size_t RightmostVertexIndex( const std::vector<Point2d> &ring )
        {
            ...
            for( std::size_t i = 1; i < ring.size(); ++i )
            {
                if( ring[i].x > ring[index].x ||
                    ( std::abs( ring[i].x - ring[index].x ) <= Geometry::kMeshConversionDefaultEpsilon &&
                      ring[i].y < ring[index].y ) )
                {
                    index = i;
                }
            }
        ...
        [[nodiscard]] bool IsAllowedBridgeIntersection( const SegmentIntersection2d &intersection,
                                                        const Point2d &bridgeStart,
                                                        const Point2d &bridgeEnd, double eps )
        {
        ...
        [[nodiscard]] bool IsVisibleBridge( const Point2d &holeVertex, const Point2d &polygonVertex,
                                            const std::vector<Point2d> &currentPolygon,
                                            const Polygon2d &polygon, double eps )
        {
            ...
            for( std::size_t i = 0; i < currentPolygon.size(); ++i )
            {
                ...
                const SegmentIntersection2d intersection =
                    Intersect( bridge, LineSegment2d( a, b ), eps );
        ...
        [[nodiscard]] bool MergeHoleIntoPolygon( const Polygon2d &polygon,
                                                 const std::vector<Point2d> &hole,
                                                 std::vector<Point2d> &currentPolygon, double eps )
        {
        ...
        [[nodiscard]] bool BuildTriangulationContour( const Polygon2d &polygon,
                                                      std::vector<Point2d> &contour, double eps )
        {
            ...
            for( std::size_t i = 0; i < polygon.HoleCount(); ++i )
            {
                std::vector<Point2d> hole =
                    RemoveAdjacentDuplicates( RingPoints( polygon.HoleAt( i ) ), eps );
        ...
        [[nodiscard]] Point3d LiftToPlane( const Point2d &point, const FaceProjection3d &projectedFace )
        {
    ...
    PolyhedronMeshConversion3d ConvertToTriangleMesh( const PolyhedronFace3d &face, double eps )
    {
        ...
        const FaceProjection3d projectedFace =
            ProjectFaceToPolygon2d( face, GeometryTolerance3d{ eps, eps, eps } );
        ...
        return { true, MeshConversionIssue3d::None, 0,
                 TriangleMesh( vertices3d, std::move( triangles ) ) };
    }
    ...
    PolyhedronMeshConversion3d ConvertToTriangleMesh( const PolyhedronBody &body, double eps )
    {
        ...
        for( std::size_t i = 0; i < body.FaceCount(); ++i )
        {
            ...
            {
                triangles.push_back(
                    TriangleMesh::TriangleIndices{ tri[0] + offset, tri[1] + offset, tri[2] + offset } );
            }
        }

        return { true, MeshConversionIssue3d::None, 0,
                 TriangleMesh( std::move( vertices ), std::move( triangles ) ) };
    }
    ...
    PolyhedronMeshConversion3d ConvertToTriangleMesh( const BrepFace &face, double eps )
    {
        ...
        return { true, MeshConversionIssue3d::None, 0,
                 TriangleMesh( std::move( vertices3d ), std::move( triangles ) ) };
    }
                 ...
                 ++localFaceIndex, ++faceIndex )
            {
                ...
                for( const TriangleMesh::TriangleIndices &tri : faceTriangles )
                {
                    triangles.push_back( TriangleMesh::TriangleIndices{
                        vertexMap[tri[0]], vertexMap[tri[1]], vertexMap[tri[2]] } );
                }
            }
        }
