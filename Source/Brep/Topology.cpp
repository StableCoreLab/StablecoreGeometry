...
namespace Geometry
{
    namespace
    {
        ...
        void CollectRingSegments( const Polyline2d &ring, std::vector<LineSegment2d> &segments )
        {
            ...
            {
                segments.emplace_back( ring.PointAt( i ),
        ...
        [[nodiscard]] BoundaryContact2d ClassifyBoundaryContact( const Polygon2d &first,
                                                                 const Polygon2d &second, double eps )
        {
            ...
            for( const LineSegment2d &lhs : firstSegments )
            {
                for( const LineSegment2d &rhs : secondSegments )
                {
                    ...
                    {
                        const bool endpointOnFirst =
                            IsEndpointParameter( intersection.points[i].parameterOnFirst, eps );
        ...
        [[nodiscard]] bool TryFindStrictInteriorSample( const Polygon2d &polygon, double eps,
                                                        Point2d &sample )
        {
        ...
        [[nodiscard]] bool HasStrictInteriorPoint( const Polygon2d &container,
                                                   const Polygon2d &candidate, double eps )
        {
    ...
    bool ContainsPoint( const Polyline2d &ring, const Point2d &point, double eps )
    {
        ...
        return containment == PointContainment2d::Inside ||
    ...
    PolygonContainment2d Relate( const Polygon2d &first, const Polygon2d &second, double eps )
    {
        ...
        const bool secondHasStrictInteriorInFirst =
            HasStrictInteriorPoint( normalizedFirst, normalizedSecond, eps );
        const bool firstHasStrictInteriorInSecond =
            HasStrictInteriorPoint( normalizedSecond, normalizedFirst, eps );
        ...
        const BoundaryContact2d contact =
            ClassifyBoundaryContact( normalizedFirst, normalizedSecond, eps );
