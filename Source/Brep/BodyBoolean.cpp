...
namespace Geometry
{
    namespace
    {
        ...
        [[nodiscard]] BodyBooleanResult3d MakeUnsupportedResult()
        {
            ...
            result.message =
                "3D body boolean currently supports only deterministic identical/disjoint closed-body "
                "subsets "
        ...
        [[nodiscard]] bool BoundsLexicographicallyLess( const Box3d &first, const Box3d &second,
                                                        double epsilon );
        ...
        [[nodiscard]] BodyBooleanResult3d MakeMultiBodyResult( std::vector<BrepBody> bodies,
                                                               const char *message )
        {
            BodyBooleanResult3d result;
            result.issue = BodyBooleanIssue3d::None;
            std::sort( bodies.begin(), bodies.end(),
                       []( const BrepBody &first, const BrepBody &second ) {
                           return BoundsLexicographicallyLess( first.Bounds(), second.Bounds(),
        ...
        [[nodiscard]] bool HasFaces( const BrepBody &body ) { return body.FaceCount() > 0; }

        [[nodiscard]] bool HasFaces( const PolyhedronBody &body ) { return body.FaceCount() > 0; }
        ...
        {
            return box.IsValid() && box.Width() > epsilon && box.Height() > epsilon &&
                   box.Depth() > epsilon;
        }
        ...
        [[nodiscard]] bool CoordinateMatchesEitherBoundary( double value, double minValue,
        ...
        [[nodiscard]] bool AppendLoopVerticesFromBody( const BrepBody &body, const BrepLoop &loop,
                                                       std::vector<Point3d> &vertices, double epsilon )
        {
            vertices.clear();
            if( !loop.IsValid() )
            {
        ...
        [[nodiscard]] bool FaceMatchesAxisAlignedBox( const BrepBody &body, const BrepFace &face,
                                                      const Box3d &box, double epsilon, int &axis,
                                                      bool &onMaxSide )
        {
            ...
            for( const Point3d &vertex : outerVertices )
            {
                ...
                if( !CoordinateMatchesEitherBoundary( coordinate0, BoxMinAt( box, otherAxis0 ),
                                                      BoxMaxAt( box, otherAxis0 ), epsilon ) ||
                    !CoordinateMatchesEitherBoundary( coordinate1, BoxMinAt( box, otherAxis1 ),
                                                      BoxMaxAt( box, otherAxis1 ), epsilon ) )
                {
                    return false;
                }
        ...
        [[nodiscard]] bool TryExtractAxisAlignedBoxFromBrep( const BrepBody &body, double epsilon,
                                                             Box3d &box )
        {
            if( body.ShellCount() != 1 )
            {
            ...
            for( std::size_t faceIndex = 0; faceIndex < shell.FaceCount(); ++faceIndex )
            {
                ...
                if( !FaceMatchesAxisAlignedBox( body, shell.FaceAt( faceIndex ), box, epsilon, axis,
                                                onMaxSide ) )
                {
                    return false;
                }
            ...
            return seenFaces[0] && seenFaces[1] && seenFaces[2] && seenFaces[3] && seenFaces[4] &&
                   seenFaces[5];
        }

        [[nodiscard]] bool TryComputePositiveIntersectionBox( const Box3d &first, const Box3d &second,
                                                              double epsilon, Box3d &overlap )
        {
            ...
            overlap =
                Box3d::FromMinMax( Point3d{ std::max( first.MinPoint().x, second.MinPoint().x ),
                                            std::max( first.MinPoint().y, second.MinPoint().y ),
                                            std::max( first.MinPoint().z, second.MinPoint().z ) },
                                   Point3d{ std::min( first.MaxPoint().x, second.MaxPoint().x ),
        ...
        [[nodiscard]] bool TryComputeIntersectionBox( const Box3d &first, const Box3d &second,
                                                      double epsilon, Box3d &overlap )
        {
            if( !first.IsValid() || !second.IsValid() )
            {
        ...
        [[nodiscard]] PolyhedronBody BuildAxisAlignedBoxPolyhedronBody( const Box3d &box )
        {
            ...
            return PolyhedronBody( {
                PolyhedronFace3d(
                    Plane::FromPointAndNormal( Point3d{ minPoint.x, minPoint.y, minPoint.z },
                ...
                PolyhedronFace3d(
                    Plane::FromPointAndNormal( Point3d{ minPoint.x, minPoint.y, maxPoint.z },
                                               Vector3d{ 0.0, 0.0, 1.0 } ),
                    PolyhedronLoop3d( {
                ...
                PolyhedronFace3d(
                    Plane::FromPointAndNormal( Point3d{ maxPoint.x, minPoint.y, minPoint.z },
                ...
                PolyhedronFace3d(
                    Plane::FromPointAndNormal( Point3d{ minPoint.x, maxPoint.y, minPoint.z },
                                               Vector3d{ 0.0, 1.0, 0.0 } ),
                    PolyhedronLoop3d( {
                        Point3d{ minPoint.x, maxPoint.y, minPoint.z },
                        Point3d{ minPoint.x, maxPoint.y, maxPoint.z },
        ...
        [[nodiscard]] BodyBooleanResult3d MakeAxisAlignedBoxResult( const Box3d &box, double epsilon,
                                                                    const char *message )
        {
        ...
        [[nodiscard]] bool FaceMatchesAxisAlignedBox( const PolyhedronFace3d &face, const Box3d &box,
                                                      double epsilon, int &axis, bool &onMaxSide )
        {
        ...
        [[nodiscard]] bool TryExtractAxisAlignedBox( const PolyhedronBody &body, double epsilon,
                                                     Box3d &box )
        {
            ...
            for( std::size_t faceIndex = 0; faceIndex < body.FaceCount(); ++faceIndex )
            {
                int axis = -1;
                bool onMaxSide = false;
                if( !FaceMatchesAxisAlignedBox( body.FaceAt( faceIndex ), box, epsilon, axis,
                                                onMaxSide ) )
                {
                    return false;
                }
        ...
        [[nodiscard]] bool TryComputeSingleBoxUnion( const Box3d &first, const Box3d &second,
                                                     const Box3d &overlap, double epsilon,
                                                     Box3d &united )
        {
            united = Box3d::FromMinMax( Point3d{ std::min( first.MinPoint().x, second.MinPoint().x ),
                                                 std::min( first.MinPoint().y, second.MinPoint().y ),
                                                 std::min( first.MinPoint().z, second.MinPoint().z ) },
                                        Point3d{ std::max( first.MaxPoint().x, second.MaxPoint().x ),
        ...
        [[nodiscard]] bool TryComputeSingleBoxDifference( const Box3d &first, const Box3d &overlap,
                                                          double epsilon, Box3d &difference )
        {
            ...
            addCandidate( Point3d{ first.MinPoint().x, first.MinPoint().y, first.MinPoint().z },
                          Point3d{ overlap.MinPoint().x, first.MaxPoint().y, first.MaxPoint().z } );
            addCandidate( Point3d{ overlap.MaxPoint().x, first.MinPoint().y, first.MinPoint().z },
                          Point3d{ first.MaxPoint().x, first.MaxPoint().y, first.MaxPoint().z } );
            addCandidate( Point3d{ overlap.MinPoint().x, first.MinPoint().y, first.MinPoint().z },
                          Point3d{ overlap.MaxPoint().x, overlap.MinPoint().y, first.MaxPoint().z } );
            addCandidate( Point3d{ overlap.MinPoint().x, overlap.MaxPoint().y, first.MinPoint().z },
                          Point3d{ overlap.MaxPoint().x, first.MaxPoint().y, first.MaxPoint().z } );
            addCandidate( Point3d{ overlap.MinPoint().x, overlap.MinPoint().y, first.MinPoint().z },
                          Point3d{ overlap.MaxPoint().x, overlap.MaxPoint().y, overlap.MinPoint().z } );
            addCandidate( Point3d{ overlap.MinPoint().x, overlap.MinPoint().y, overlap.MaxPoint().z },
            ...
            return NearlyEqualScaled( BoxVolume( difference ), BoxVolume( first ) - BoxVolume( overlap ),
                                      epsilon );
        }

        [[nodiscard]] bool TryDetectFaceTouchingDifferenceIdentity( const Box3d &first,
                                                                    const Box3d &second, double epsilon )
        {
            if( !first.IsValid() || !second.IsValid() )
            {
                return false;
        ...
        [[nodiscard]] bool BoxContains( const Box3d &container, const Box3d &containee, double epsilon )
        {
        ...
        [[nodiscard]] bool TryDetectNonVolumeTouchingIntersectionEmpty( const Box3d &first,
                                                                        const Box3d &second,
                                                                        double epsilon )
        {
            ...
            for( int axis = 0; axis < 3; ++axis )
            {
                ...
                if( NearlyEqual( firstMax, secondMin, epsilon ) ||
                    NearlyEqual( secondMax, firstMin, epsilon ) )
                {
                    ++touchingAxes;
                    continue;
                }
        ...
        [[nodiscard]] bool TryDetectEdgeOrVertexTouchingUnionAsMultiBody( const Box3d &first,
                                                                          const Box3d &second,
                                                                          double epsilon )
        {
        ...
        [[nodiscard]] bool BodiesLookIdentical( const BrepBody &first, const BrepBody &second,
                                                double epsilon )
        {
        ...
        [[nodiscard]] BodyBooleanResult3d IntersectClosedSubset( const BrepBody &first,
                                                                 ...
                                                                 const BodyBooleanOptions3d &options )
        {
            ...
            if( BodiesLookIdentical( first, second, epsilon ) )
            {
                return MakeSingleBodyResult( first,
                                             "Deterministic identical-body intersection subset." );
            }
                ...
                TryExtractAxisAlignedBox( second, epsilon, secondBox ) )
            {
                if( BoxContains( firstBox, secondBox, epsilon ) )
                {
                    return MakeSingleBodyResult(
                        second, "Deterministic axis-aligned contained-body intersection subset." );
                }

                if( BoxContains( secondBox, firstBox, epsilon ) )
                {
                    return MakeSingleBodyResult(
                        first, "Deterministic axis-aligned contained-body intersection subset." );
                }
            }
                ...
                TryComputePositiveIntersectionBox( firstBox, secondBox, epsilon, overlapBox ) )
            {
                return MakeAxisAlignedBoxResult(
                    overlapBox, epsilon, "Deterministic axis-aligned overlap-box intersection subset." );
            }
                ...
                TryDetectNonVolumeTouchingIntersectionEmpty( firstBox, secondBox, epsilon ) )
            {
                return MakeEmptyResult(
                    "Deterministic axis-aligned touching empty intersection subset." );
            }

            return MakeUnsupportedResult();
        }

        [[nodiscard]] BodyBooleanResult3d UnionClosedSubset( const BrepBody &first,
                                                             const BrepBody &second,
                                                             const BodyBooleanOptions3d &options )
        {
            const double epsilon = ResolveTolerance( options );
            ...
            if( BoundsDisjoint( first.Bounds(), second.Bounds(), epsilon ) )
            {
                return MakeMultiBodyResult( { first, second },
                                            "Deterministic disjoint-body union subset." );
            }
                ...
                TryExtractAxisAlignedBox( second, epsilon, secondBox ) )
            {
                ...
                {
                    return MakeSingleBodyResult(
                        first, "Deterministic axis-aligned contained-body union subset." );
                }

                if( BoxContains( secondBox, firstBox, epsilon ) )
                {
                    return MakeSingleBodyResult(
                        second, "Deterministic axis-aligned contained-body union subset." );
                }
            }
        ...
        [[nodiscard]] BodyBooleanResult3d DifferenceClosedSubset( const BrepBody &first,
                                                                  const BrepBody &second,
                                                                  const BodyBooleanOptions3d &options )
        {
            const double epsilon = ResolveTolerance( options );
            ...
            {
                return MakeEmptyResult(
                    "Deterministic axis-aligned contained difference empty subset." );
            }
            ...
            {
                return MakeSingleBodyResult( first,
                                             "Deterministic face-touching external difference subset." );
            }
            ...
            {
                return MakeSingleBodyResult(
                    first, "Deterministic edge/vertex-touching external difference subset." );
            }

            return MakeUnsupportedResult();
        }

        [[nodiscard]] BodyBooleanResult3d MakeResultForBrepBodies( const BrepBody &first,
                                                                   const BrepBody &second,
                                                                   const BodyBooleanOptions3d &options,
        ...
        [[nodiscard]] BodyBooleanResult3d MakeResultForPolyhedronBodies(
            const PolyhedronBody &first, const PolyhedronBody &second,
            const BodyBooleanOptions3d &options, const char operation )
        {
            if( !HasFaces( first ) || !HasFaces( second ) )
            {
                return MakeInvalidInputResult();
            ...
            return MakeResultForBrepBodies( firstConversion.body, secondConversion.body, options,
                                            operation );
        }
    }  // namespace

    BodyBooleanResult3d IntersectBodies( const BrepBody &first, const BrepBody &second,
                                         BodyBooleanOptions3d options )
    {
        return MakeResultForBrepBodies( first, second, options, 'i' );
    }

    BodyBooleanResult3d UnionBodies( const BrepBody &first, const BrepBody &second,
                                     BodyBooleanOptions3d options )
    {
        return MakeResultForBrepBodies( first, second, options, 'u' );
    }

    BodyBooleanResult3d DifferenceBodies( const BrepBody &first, const BrepBody &second,
                                          BodyBooleanOptions3d options )
    {
        return MakeResultForBrepBodies( first, second, options, 'd' );
    }

    BodyBooleanResult3d IntersectBodies( const PolyhedronBody &first, const PolyhedronBody &second,
                                         BodyBooleanOptions3d options )
    {
        return MakeResultForPolyhedronBodies( first, second, options, 'i' );
    }

    BodyBooleanResult3d UnionBodies( const PolyhedronBody &first, const PolyhedronBody &second,
                                     BodyBooleanOptions3d options )
    {
        return MakeResultForPolyhedronBodies( first, second, options, 'u' );
    }

    BodyBooleanResult3d DifferenceBodies( const PolyhedronBody &first, const PolyhedronBody &second,
                                          BodyBooleanOptions3d options )
    {
