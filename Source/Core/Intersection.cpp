...
namespace Geometry
{
    namespace
    {
        ...
        [[nodiscard]] SegmentIntersection2d IntersectLineLineInternal( const LineSegment2d &first,
                                                                       const LineSegment2d &second,
                                                                       double eps );

        [[nodiscard]] SegmentIntersection2d IntersectLineArcInternal( const LineSegment2d &line,
                                                                      const ArcSegment2d &arc,
                                                                      double eps );

        [[nodiscard]] SegmentIntersection2d IntersectArcArcInternal( const ArcSegment2d &first,
                                                                     const ArcSegment2d &second,
                                                                     double eps );

        [[nodiscard]] ArcProjectionCandidate ProjectPointToArcSegmentLocal( const Point2d &point,
                                                                            const ArcSegment2d &arc,
                                                                            double eps );
        ...
        [[nodiscard]] Point2d MakePoint( double x, double y ) { return Point2d{ x, y }; }

        [[nodiscard]] Point2d PointAtAngle( const ArcSegment2d &arc, double angle )
        {
            return MakePoint( arc.center.x + arc.radius * std::cos( angle ),
        ...
        [[nodiscard]] SegmentProjection2d ProjectPointToLineSegmentLocal( const Point2d &point,
                                                                          ...
                                                                          bool clampToSegment = true )
        {
                ...
                Geometry::kIntersectionDefaultEpsilon * Geometry::kIntersectionDefaultEpsilon )
            {
                return SegmentProjection2d{ segment.startPoint, 0.0,
                                            DistanceSquaredPoints( point, segment.startPoint ), true };
            }
            ...
            return SegmentProjection2d{
                projectedPoint, parameter, DistanceSquaredPoints( point, projectedPoint ),
                                                                            ...
                                                                            double eps )
        {
            ...
            {
                return ArcProjectionCandidate{ PointAtAngle( arc, angle ),
        ...
        [[nodiscard]] ClosestPoints2d MakeClosestPoints( const Point2d &firstPoint,
                                                         const Point2d &secondPoint,
                                                         double firstParameter, double secondParameter )
        {
            return ClosestPoints2d{ firstPoint, secondPoint, firstParameter, secondParameter,
        ...
        void UpdateClosest( ClosestPoints2d &best, bool &hasBest, const Point2d &firstPoint,
                            const Point2d &secondPoint, double firstParameter, double secondParameter )
        {
            const double distanceSquared = DistanceSquaredPoints( firstPoint, secondPoint );
            if( !hasBest ||
                distanceSquared + Geometry::kIntersectionDefaultEpsilon < best.distanceSquared )
            {
                best = MakeClosestPoints( firstPoint, secondPoint, firstParameter, secondParameter,
                                          distanceSquared );
                hasBest = true;
            }
        }

        [[nodiscard]] SegmentIntersection2d MakeNoIntersection() { return SegmentIntersection2d{}; }

        void AddIntersectionPoint( SegmentIntersection2d &result, IntersectionKind2d kind,
                                   const Point2d &point, double parameterOnFirst,
                                   double parameterOnSecond, double eps )
        {
        ...
        [[nodiscard]] SegmentIntersection2d MakePointIntersection(
            const Point2d &point, double firstParameter, double secondParameter,
        ...
        [[nodiscard]] SegmentIntersection2d MakeOverlapIntersection(
            const IntersectionPoint2d &firstPoint, const IntersectionPoint2d &secondPoint )
        {
            SegmentIntersection2d result;
                                                                       ...
                                                                       double eps )
        {
            ...
            if( AlmostEqual( rxs, 0.0, eps ) )
            {
                ...
                {
                    return MakePointIntersection( firstPoint, overlapStart, firstParameterOnSecond,
                                                                      ...
                                                                      double eps )
        {
            ...
            for( double root : roots )
            {
                ...
                if( !hasPoint )
                {
                    result = MakePointIntersection( point, parameter, arcParameter,
                                                    AlmostEqual( discriminant, 0.0, eps )
                ...
                AddIntersectionPoint( result, IntersectionKind2d::Point, point, parameter, arcParameter,
                                      eps );
            }
        ...
        [[nodiscard]] bool SameCircle( const ArcSegment2d &first, const ArcSegment2d &second,
                                       double eps )
        {
        ...
        [[nodiscard]] SegmentIntersection2d IntersectArcArcSameCircle( const ArcSegment2d &first,
                                                                       const ArcSegment2d &second,
                                                                       double eps )
        {
            ...
            {
                return MakeOverlapIntersection( IntersectionPoint2d{ first.StartPoint(), 0.0, 0.0 },
            ...
            auto addCandidate = [&]( const Point2d &point, double parameterOnFirst,
                ...
                if( candidateCount < candidates.size() )
                {
                    candidates[candidateCount++] =
                        IntersectionPoint2d{ point, parameterOnFirst, parameterOnSecond };
                }
            };
            ...
            if( candidateCount == 1 )
            {
                return MakePointIntersection( candidates[0].point, candidates[0].parameterOnFirst,
        ...
        [[nodiscard]] ClosestPoints2d ClosestPointsLineLineInternal( const LineSegment2d &first,
                                                                     const LineSegment2d &second )
        {
            ...
            if( intersection.HasIntersection() )
            {
                ...
                return ClosestPoints2d{ point.point, point.point, point.parameterOnFirst,
                                        point.parameterOnSecond, 0.0 };
            }

            ClosestPoints2d best{};
            bool hasBest = false;

            const SegmentProjection2d firstStartProjection =
                ProjectPointToLineSegmentLocal( first.startPoint, second, true );
            UpdateClosest( best, hasBest, first.startPoint, firstStartProjection.point, 0.0,
                           firstStartProjection.parameter );

            const SegmentProjection2d firstEndProjection =
                ProjectPointToLineSegmentLocal( first.endPoint, second, true );
            UpdateClosest( best, hasBest, first.endPoint, firstEndProjection.point, 1.0,
                           firstEndProjection.parameter );

            const SegmentProjection2d secondStartProjection =
                ProjectPointToLineSegmentLocal( second.startPoint, first, true );
            UpdateClosest( best, hasBest, secondStartProjection.point, second.startPoint,
                           secondStartProjection.parameter, 0.0 );

            const SegmentProjection2d secondEndProjection =
                ProjectPointToLineSegmentLocal( second.endPoint, first, true );
            UpdateClosest( best, hasBest, secondEndProjection.point, second.endPoint,
                           secondEndProjection.parameter, 1.0 );

            return best;
        }

        [[nodiscard]] ClosestPoints2d ClosestPointsLineArcInternal( const LineSegment2d &line,
                                                                    const ArcSegment2d &arc )
        {
            const SegmentIntersection2d intersection =
            ...
            const auto updateFromLineEndpoint = [&]( const Point2d &endpoint, double parameterOnLine ) {
                const ArcProjectionCandidate projected = ProjectPointToArcSegmentLocal(
                    endpoint, arc, Geometry::kIntersectionDefaultEpsilon );
                UpdateClosest( best, hasBest, endpoint, projected.point, parameterOnLine,
                               projected.parameter );
            };

            const auto updateFromArcEndpoint = [&]( const Point2d &endpoint, double parameterOnArc ) {
                const SegmentProjection2d projection =
                    ProjectPointToLineSegmentLocal( endpoint, line, true );
                UpdateClosest( best, hasBest, projection.point, endpoint, projection.parameter,
                               parameterOnArc );
            };
            ...
            const SegmentProjection2d centerProjection =
                ProjectPointToLineSegmentLocal( arc.center, line, true );
                ...
                Geometry::kIntersectionDefaultEpsilon * Geometry::kIntersectionDefaultEpsilon )
            {
                ...
                if( IsAngleOnArc( arc, angle, Geometry::kIntersectionDefaultEpsilon ) )
                {
                    ...
                    UpdateClosest( best, hasBest, centerProjection.point, arcPoint,
                                   centerProjection.parameter, ArcParameterAtAngle( arc, angle ) );
                }
            }
        ...
        [[nodiscard]] ClosestPoints2d ClosestPointsArcArcInternal( const ArcSegment2d &first,
                                                                   const ArcSegment2d &second )
        {
            const SegmentIntersection2d intersection =
            ...
            const auto updateFromFirstEndpoint = [&]( const Point2d &endpoint,
                                                      double parameterOnFirst ) {
                const ArcProjectionCandidate projected = ProjectPointToArcSegmentLocal(
                    endpoint, second, Geometry::kIntersectionDefaultEpsilon );
                UpdateClosest( best, hasBest, endpoint, projected.point, parameterOnFirst,
                               projected.parameter );
            };

            const auto updateFromSecondEndpoint = [&]( const Point2d &endpoint,
                                                       double parameterOnSecond ) {
                const ArcProjectionCandidate projected = ProjectPointToArcSegmentLocal(
                    endpoint, first, Geometry::kIntersectionDefaultEpsilon );
                UpdateClosest( best, hasBest, projected.point, endpoint, projected.parameter,
                               parameterOnSecond );
            };
            ...
            if( centerDistanceSquared >
                Geometry::kIntersectionDefaultEpsilon * Geometry::kIntersectionDefaultEpsilon )
            {
                const double centerDistance = std::sqrt( centerDistanceSquared );
                const Vector2d unit = centerDelta / centerDistance;
                const Point2d firstCandidate = MakePoint( first.center.x + first.radius * unit.x,
                                                          first.center.y + first.radius * unit.y );
                const Point2d secondCandidate = MakePoint( second.center.x - second.radius * unit.x,
                ...
                const double secondAngle = std::atan2( secondCandidate.y - second.center.y,
                                                       secondCandidate.x - second.center.x );
                ...
                {
                    UpdateClosest( best, hasBest, firstCandidate, secondCandidate,
                                                                     ...
                                                                     double eps )
        {
            ...
            const double a =
                ( first.radius * first.radius - second.radius * second.radius + distanceSquared ) /
                ( 2.0 * distance );
            ...
            const Point2d basePoint =
                MakePoint( first.center.x + a * unit.x, first.center.y + a * unit.y );
            const Vector2d perpendicular{ -unit.y, unit.x };

            const Point2d candidateOne =
                MakePoint( basePoint.x + h * perpendicular.x, basePoint.y + h * perpendicular.y );
            const Point2d candidateTwo =
                MakePoint( basePoint.x - h * perpendicular.x, basePoint.y - h * perpendicular.y );
            ...
            {
                const double angleFirst =
                    std::atan2( point.y - first.center.y, point.x - first.center.x );
                const double angleSecond =
                    std::atan2( point.y - second.center.y, point.x - second.center.x );
                if( !IsAngleOnArc( first, angleFirst, eps ) ||
                    !IsAngleOnArc( second, angleSecond, eps ) )
                {
                    continue;
                }
                ...
                {
                    result =
                        MakePointIntersection( point, firstParameter, secondParameter,
                                               AlmostEqual( h, 0.0, eps ) ? IntersectionKind2d::Tangent
                                                                          : IntersectionKind2d::Point );
                    hasPoint = true;
                    continue;
                }

                AddIntersectionPoint( result, IntersectionKind2d::Point, point, firstParameter,
                                      secondParameter, eps );
            }
        ...
        [[nodiscard]] ClosestPoints2d ClosestPointsLineArcInternal( const LineSegment2d &line,
                                                                    const ArcSegment2d &arc, double eps )
        {
            ...
            const auto updateFromLineEndpoint = [&]( const Point2d &endpoint, double parameterOnLine ) {
                const ArcProjectionCandidate projected =
                    ProjectPointToArcSegmentLocal( endpoint, arc, eps );
                UpdateClosest( best, hasBest, endpoint, projected.point, parameterOnLine,
                               projected.parameter );
            };
    ...
    SegmentIntersection2d Intersect( const LineSegment2d &first, const LineSegment2d &second,
                                     double eps )
    {
        return IntersectLineLineInternal( first, second, eps );
    }

    SegmentIntersection2d Intersect( const LineSegment2d &first, const ArcSegment2d &second, double eps )
    {
        return IntersectLineArcInternal( first, second, eps );
    }

    SegmentIntersection2d Intersect( const ArcSegment2d &first, const ArcSegment2d &second, double eps )
    {
        return IntersectArcArcInternal( first, second, eps );
    }

    SegmentIntersection2d Intersect( const Segment2d &first, const Segment2d &second, double eps )
    {
        if( !first.IsValid() || !second.IsValid() )
        {
        ...
        {
            return Intersect( static_cast<const LineSegment2d &>( first ),
                              static_cast<const LineSegment2d &>( second ), eps );
        }

        if( first.Kind() == SegmentKind2::Line && second.Kind() == SegmentKind2::Arc )
        {
            return Intersect( static_cast<const LineSegment2d &>( first ),
                              static_cast<const ArcSegment2d &>( second ), eps );
        }

        if( first.Kind() == SegmentKind2::Arc && second.Kind() == SegmentKind2::Line )
        {
            return SwapIntersectionResult( Intersect( static_cast<const LineSegment2d &>( second ),
        ...
        {
            return Intersect( static_cast<const ArcSegment2d &>( first ),
                              static_cast<const ArcSegment2d &>( second ), eps );
        }
    ...
    bool HasIntersection( const Segment2d &first, const Segment2d &second, double eps )
    {
        return Intersect( first, second, eps ).HasIntersection();
    }

    ClosestPoints2d ClosestPoints( const LineSegment2d &first, const LineSegment2d &second )
    {
        return ClosestPointsLineLineInternal( first, second );
    }

    ClosestPoints2d ClosestPoints( const LineSegment2d &first, const ArcSegment2d &second )
    {
        return ClosestPointsLineArcInternal( first, second, Geometry::kIntersectionDefaultEpsilon );
    }

    ClosestPoints2d ClosestPoints( const ArcSegment2d &first, const ArcSegment2d &second )
    {
        return ClosestPointsArcArcInternal( first, second );
    }

    ClosestPoints2d ClosestPoints( const Segment2d &first, const Segment2d &second )
    {
        if( !first.IsValid() || !second.IsValid() )
        {
        ...
        if( first.Kind() == SegmentKind2::Line && second.Kind() == SegmentKind2::Line )
        {
            return ClosestPoints( static_cast<const LineSegment2d &>( first ),
        ...
        if( first.Kind() == SegmentKind2::Arc && second.Kind() == SegmentKind2::Line )
        {
            return SwapClosestPointsResult(
                ClosestPoints( static_cast<const LineSegment2d &>( second ),
                               static_cast<const ArcSegment2d &>( first ) ) );
        }

        if( first.Kind() == SegmentKind2::Arc && second.Kind() == SegmentKind2::Arc )
        {
            return ClosestPoints( static_cast<const ArcSegment2d &>( first ),
                                  static_cast<const ArcSegment2d &>( second ) );
        }
