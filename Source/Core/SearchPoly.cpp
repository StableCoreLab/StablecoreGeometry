...
namespace Geometry
{
    namespace
    {
        ...
        [[nodiscard]] std::size_t FindOrAddVertex( std::vector<GraphVertex2d> &vertices,
                                                   const Point2d &point, double epsilon );
        ...
        [[nodiscard]] std::vector<GraphVertex2d> BuildSegmentVertices(
            const std::vector<LineSegment2d> &segments, double epsilon )
        {
            std::vector<GraphVertex2d> vertices;
        ...
        [[nodiscard]] double ComputeRepairToleranceFromSegments(
            const std::vector<LineSegment2d> &segments, double epsilon )
        {
        ...
        [[nodiscard]] LineNetworkAnalysis2d AnalyzeLineNetwork( const MultiPolyline2d &lines,
                                                                double epsilon )
        {
            ...
            for( std::size_t lineIndex = 0; lineIndex < lines.Count(); ++lineIndex )
            {
                ...
                const std::size_t segmentCount =
                    line.IsClosed() ? line.PointCount() : line.PointCount() - 1;
            ...
            analysis.diagnostics.inferredSyntheticEdgeCount =
                analysis.diagnostics.danglingEndpointCount / 2U;
            analysis.repairTolerance = ComputeRepairToleranceFromSegments( analysis.segments, epsilon );
            return analysis;
        }

        [[nodiscard]] double ComputeBranchScore( double area, double inferredSyntheticPerimeter,
                                                 std::size_t branchVertexCount,
                                                 std::size_t syntheticBranchVertexCount,
                                                 double repairTolerance, double epsilon )
        {
            ...
            const double syntheticBranchPenalty =
                static_cast<double>( syntheticBranchVertexCount ) * 1.5 * scale;
        ...
        [[nodiscard]] SearchPolySyntheticEdgeKind2d ClassifySyntheticEdgeKind(
            const LineNetworkAnalysis2d &analysis, const Point2d &start, const Point2d &end,
            double epsilon )
        {
            ...
            const std::vector<SearchPolySyntheticEdgeKind2d> &syntheticEdgeKinds )
        {
            ...
            const std::size_t maxCount = std::max( std::max( gapClosureCount, branchCleanupCount ),
            ...
            const std::vector<SearchPolySyntheticEdgeSource2d> &syntheticEdgeSources )
        {
            ...
            const std::size_t maxCount = std::max( std::max( singleGapCloseCount, branchCleanupCount ),
                                                   std::max( mixedBridgeCount, unknownCount ) );
            const std::size_t winnerCount = ( singleGapCloseCount == maxCount ? 1U : 0U ) +
                                            ( branchCleanupCount == maxCount ? 1U : 0U ) +
        ...
        [[nodiscard]] bool CoversBoundaryEdge( const LineSegment2d &boundaryEdge,
                                               ...
                                               double epsilon )
        {
            ...
            for( const LineSegment2d &inputSegment : inputSegments )
            {
                ...
                const double endOffset =
                    std::abs( Cross( direction, inputSegment.endPoint - boundaryEdge.startPoint ) );
                ...
                double t0 =
                    Dot( inputSegment.startPoint - boundaryEdge.startPoint, direction ) / lengthSquared;
                double t1 =
                    Dot( inputSegment.endPoint - boundaryEdge.startPoint, direction ) / lengthSquared;
                if( t1 < t0 )
                {
                    std::swap( t0, t1 );
                }
        ...
        [[nodiscard]] MultiPolyline2d BuildSegmentPolylines( const std::vector<LineSegment2d> &segments )
        {
            ...
            for( const LineSegment2d &segment : segments )
            {
                ...
                polylines.Add(
                    Polyline2d( { segment.startPoint, segment.endPoint }, PolylineClosure::Open ) );
            }
            return polylines;
        }

        [[nodiscard]] std::vector<LineSegment2d> PruneBranchDanglingSegments(
            std::vector<LineSegment2d> segments, double epsilon )
        {
            bool changed = true;
            while( changed )
            {
                ...
                for( const LineSegment2d &segment : segments )
                {
                    const std::size_t startDegree =
                        BranchDegreeAtPoint( vertices, segment.startPoint, epsilon );
                    const std::size_t endDegree =
                        BranchDegreeAtPoint( vertices, segment.endPoint, epsilon );
        ...
        [[nodiscard]] MultiPolygon2d TryBuildPolygonsWithBranchCleanupFallback(
            const LineNetworkAnalysis2d &analysis, const SearchPolyOptions2d &options )
        {
            if( !options.removeBranches || !options.allowFakeEdges ||
                analysis.diagnostics.branchVertexCount == 0U )
            {
                return {};
            }
            ...
            MultiPolygon2d polygons =
                BuildMultiPolygonByLines( BuildSegmentPolylines( prunedSegments ), options.epsilon );
            if( polygons.Count() > 0 )
            {
                return polygons;
            }
            ...
            const std::vector<GraphVertex2d> vertices =
                BuildSegmentVertices( prunedSegments, options.epsilon );
        ...
        void AccumulateRingMetrics( const Polyline2d &ring, const LineNetworkAnalysis2d &analysis,
                                    double epsilon, CandidateMetrics2d &metrics )
        {
            ...
            for( std::size_t segmentIndex = 0; segmentIndex < segmentCount; ++segmentIndex )
            {
                ...
                const bool inferredSynthetic =
                    !CoversBoundaryEdge( boundaryEdge, analysis.segments, epsilon );
                if( inferredSynthetic )
                {
                    const std::size_t startVertexIndex =
                        FindVertexIndex( analysis.vertices, start, epsilon );
                    const std::size_t endVertexIndex =
                        FindVertexIndex( analysis.vertices, end, epsilon );
                    const std::size_t startDegree =
                        BranchDegreeAtPoint( analysis.vertices, start, epsilon );
                    const std::size_t endDegree = BranchDegreeAtPoint( analysis.vertices, end, epsilon );
        ...
        [[nodiscard]] CandidateMetrics2d AnalyzeCandidate( const Polygon2d &polygon,
                                                           ...
                                                           double epsilon )
        {
            ...
            metrics.branchScore = ComputeBranchScore(
                std::abs( polygonArea ), metrics.inferredSyntheticPerimeter, metrics.branchVertexCount,
                metrics.syntheticBranchVertexCount, analysis.repairTolerance, epsilon );
            return metrics;
        }
                                                           ...
                                                           const CandidateMetrics2d &metrics )
        {
            ...
            const double polygonArea = polygon.Area();
            return SearchPolyCandidate2d{ polygon,
                                          std::abs( polygonArea ),
                                          metrics.branchScore,
        ...
        void RankCandidates( std::vector<SearchPolyCandidate2d> &candidates )
        {
            std::stable_sort(
                ...
                []( const SearchPolyCandidate2d &left, const SearchPolyCandidate2d &right ) {
                    if( std::abs( left.branchScore - right.branchScore ) >
                        Geometry::kSearchPolyComparisonEpsilon )
                    {
                        return left.branchScore > right.branchScore;
                    }
                    if( std::abs( left.absoluteArea - right.absoluteArea ) >
                        Geometry::kSearchPolyComparisonEpsilon )
                    {
                    ...
                    return left.polygon.OuterRing().PointCount() <
                           right.polygon.OuterRing().PointCount();
                } );
        ...
        void PopulateResultExplanation( SearchPolyResult2d &result )
        {
            ...
            std::vector<SearchPolyPenaltyKind2d> ambiguousTopPenaltyKinds{
                bestCandidate.dominantPenaltyKind
            };
            std::vector<SearchPolySyntheticEdgeKind2d> ambiguousTopSyntheticKinds{
                bestCandidate.dominantSyntheticEdgeKind
            };
            std::vector<SearchPolySyntheticEdgeSource2d> ambiguousTopSyntheticSources{
            ...
            result.ambiguousTopSyntheticEdgeKind =
                DetermineDominantSyntheticEdgeKind( ambiguousTopSyntheticKinds );
            result.ambiguousTopSyntheticEdgeSource =
                DetermineDominantSyntheticEdgeSource( ambiguousTopSyntheticSources );
            ...
            if( result.candidates.size() >= 2U )
            {
                ...
                result.runnerUpBranchVertexCount =
                    runnerUp.branchVertexCount + runnerUp.syntheticBranchVertexCount;
                ...
                result.bestCandidateBeatsBranchRunnerUp =
                    bestBranchPenaltyCount < result.runnerUpBranchVertexCount;
            }
        }
    }  // namespace
    ...
    SearchPolyResult2d SearchPolygons( const MultiPolyline2d &lines, SearchPolyOptions2d options )
    {
        ...
        for( std::size_t index = 0; index < result.polygons.Count(); ++index )
        {
            if( result.polygons[index].IsValid() )
            {
                result.candidates.push_back( MakeCandidate(
                    result.polygons[index],
                    AnalyzeCandidate( result.polygons[index], analysis, options.epsilon ) ) );
            }
        }
        ...
        result.usedSyntheticEdges = std::any_of( result.candidates.begin(), result.candidates.end(),
                                                 []( const SearchPolyCandidate2d &candidate ) {
                                                     return candidate.inferredSyntheticEdgeCount > 0;
                                                 } );
        result.usedBranchScoring = std::any_of( result.candidates.begin(), result.candidates.end(),
                                                []( const SearchPolyCandidate2d &candidate ) {
                                                    return candidate.inferredSyntheticEdgeCount > 0 ||
                                                           candidate.branchVertexCount > 0 ||
    ...
    std::optional<SearchPolyCandidate2d> SearchPolygonContainingPoint( const MultiPolyline2d &lines,
                                                                       const Point2d &point,
                                                                       SearchPolyOptions2d options )
    {
        ...
        for( const SearchPolyCandidate2d &candidate : result.candidates )
        {
            const PointContainment2d containment =
                LocatePoint( point, candidate.polygon, options.epsilon );
