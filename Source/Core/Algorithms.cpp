...
namespace Geometry
{
    namespace
    {
        [[nodiscard]] Geometry::Point2d Interpolate( const Geometry::Point2d &startPoint,
                                                     const Geometry::Point2d &endPoint,
                                                     double parameter )
        {
            return Geometry::Point2d( startPoint.x + ( endPoint.x - startPoint.x ) * parameter,
                                      startPoint.y + ( endPoint.y - startPoint.y ) * parameter );
        }
    }  // namespace

    SegmentProjection2d ProjectPointToSegment( const Point2d &point, const Point2d &segmentStart,
                                               const Point2d &segmentEnd, bool clampToSegment )
    {
        ...
        {
            return SegmentProjection2d{ segmentStart, 0.0, DistanceSquared( point, segmentStart ),
                                        true };
        }
