...
namespace Geometry
{
    ...
    double DistanceSquared( const Point3d &point, const Line3d &line,
    ...
    double DistanceSquared( const Point3d &point, const Curve3d &curve,
                            const GeometryTolerance3d &tolerance )
    {
    ...
    double DistanceSquared( const Point3d &point, const CurveOnSurface &curveOnSurface,
                            const GeometryTolerance3d &tolerance )
    {
        return ProjectPointToCurveOnSurface( point, curveOnSurface, tolerance ).distanceSquared;
    }

    double Distance( const Point3d &point, const CurveOnSurface &curveOnSurface,
                     const GeometryTolerance3d &tolerance )
    {
        return std::sqrt( DistanceSquared( point, curveOnSurface, tolerance ) );
    }

    double DistanceSquared( const Point3d &point, const Surface &surface,
                            const GeometryTolerance3d &tolerance )
    {
    ...
    double DistanceSquared( const Point3d &point, const BrepFace &face,
                            const GeometryTolerance3d &tolerance )
    {
    ...
    double DistanceSquared( const Point3d &point, const BrepEdge &edge,
                            const GeometryTolerance3d &tolerance )
    {
    ...
    double DistanceSquared( const Point3d &point, const BrepVertex &vertex,
                            const GeometryTolerance3d &tolerance )
    {
        return ProjectPointToBrepVertex( point, vertex, tolerance ).distanceSquared;
    }

    double Distance( const Point3d &point, const BrepVertex &vertex,
                     const GeometryTolerance3d &tolerance )
    {
        return std::sqrt( DistanceSquared( point, vertex, tolerance ) );
    }

    double DistanceSquared( const Point3d &point, const BrepBody &body,
                            const GeometryTolerance3d &tolerance )
    {
    ...
    double DistanceSquared( const Point3d &point, const PolyhedronFace3d &face,
                            const GeometryTolerance3d &tolerance )
    {
        return ProjectPointToPolyhedronFace( point, face, tolerance ).distanceSquared;
    }

    double Distance( const Point3d &point, const PolyhedronFace3d &face,
                     const GeometryTolerance3d &tolerance )
    {
        return std::sqrt( DistanceSquared( point, face, tolerance ) );
    }

    double DistanceSquared( const Point3d &point, const PolyhedronBody &body,
                            const GeometryTolerance3d &tolerance )
    {
        return ProjectPointToPolyhedronBody( point, body, tolerance ).projection.distanceSquared;
    }

    double Distance( const Point3d &point, const PolyhedronBody &body,
                     const GeometryTolerance3d &tolerance )
    {
        return std::sqrt( DistanceSquared( point, body, tolerance ) );
    }

    double DistanceSquared( const Point3d &point, const TriangleMesh &mesh,
                            const GeometryTolerance3d &tolerance )
    {
        return ProjectPointToTriangleMesh( point, mesh, tolerance ).distanceSquared;
    }

    double Distance( const Point3d &point, const TriangleMesh &mesh,
                     const GeometryTolerance3d &tolerance )
    {
    ...
    double Area( const Triangle3d &triangle ) { return triangle.Area(); }
    ...
    double Area( const TriangleMesh &mesh ) { return mesh.SurfaceArea(); }
    ...
    double Volume( const TriangleMesh &mesh )
    {
        ...
        for( std::size_t i = 0; i < mesh.TriangleCount(); ++i )
        {
            ...
            signedVolume +=
                Dot( triangle.a - Point3d{}, Cross( triangle.b - Point3d{}, triangle.c - Point3d{} ) ) /
                6.0;
        }
    ...
    Box3d Bounds( const Curve3d &curve ) { return curve.Bounds(); }

    Box3d Bounds( const Surface &surface ) { return surface.Bounds(); }

    Box3d Bounds( const CurveOnSurface &curveOnSurface ) { return curveOnSurface.Bounds(); }

    Box3d Bounds( const TriangleMesh &mesh ) { return mesh.Bounds(); }

    Box3d Bounds( const PolyhedronFace3d &face ) { return face.Bounds(); }

    Box3d Bounds( const PolyhedronBody &body ) { return body.Bounds(); }
    ...
    Box3d Bounds( const BrepEdge &edge ) { return edge.Bounds(); }

    Box3d Bounds( const BrepFace &face ) { return face.Bounds(); }

    Box3d Bounds( const BrepBody &body ) { return body.Bounds(); }
}  // namespace Geometry
