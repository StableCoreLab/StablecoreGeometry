...
namespace Geometry
{
    ArcSegment2d::ArcSegment2d( const Point2d &center, double radius, double startAngle,
                                double sweepAngle ) :
        center( center ),
        radius( radius ),
    ...
    ArcSegment2d::ArcSegment2d( const Point2d &center, double radius, double startAngle, double endAngle,
    ...
    ArcSegment2d ArcSegment2d::FromCenterRadiusStartSweep( const Point2d &center, double radius,
                                                           double startAngle, double sweepAngle )
    {
        return ArcSegment2d( center, radius, startAngle, sweepAngle );
    }

    SegmentKind2 ArcSegment2d::Kind() const { return SegmentKind2::Arc; }

    bool ArcSegment2d::IsValid() const
    {
        return center.IsValid() && std::isfinite( radius ) && std::isfinite( startAngle ) &&
               std::isfinite( sweepAngle ) && radius > 0.0 &&
               std::abs( sweepAngle ) > Geometry::kArcSegmentDefaultEpsilon &&
               std::abs( sweepAngle ) <=
                   2.0 * std::numbers::pi_v<double> + Geometry::kArcSegmentDefaultEpsilon;
    }
    ...
    double ArcSegment2d::EndAngle() const { return startAngle + sweepAngle; }

    Point2d ArcSegment2d::StartPoint() const { return PointAtAngle( startAngle ); }

    Point2d ArcSegment2d::EndPoint() const { return PointAtAngle( EndAngle() ); }
    ...
    Box2d ArcSegment2d::Bounds() const
    {
        ...
        constexpr double kCriticalAngles[] = { 0.0, std::numbers::pi_v<double> * 0.5,
    ...
    std::string ArcSegment2d::DebugString() const
    {
        ...
        stream << "ArcSegment2d{center=" << center.DebugString() << ", radius=" << radius
