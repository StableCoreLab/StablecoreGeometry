...
namespace Geometry
{
    namespace
    {
        ...
        [[nodiscard]] std::unique_ptr<KDNode> BuildKDTree( const std::vector<KDTreeEntry2d> &entries,
        ...
        void QueryExact( const KDNode *node, const std::vector<KDTreeEntry2d> &entries,
        ...
        void QueryRadius( const KDNode *node, const std::vector<KDTreeEntry2d> &entries,
                          const Point2d &point, double maxDistanceSquared,
        ...
        void QueryNearest( const KDNode *node, const std::vector<KDTreeEntry2d> &entries,
                           const Point2d &point, std::optional<KDTreeHit2d> &best )
        {
            if( node == nullptr )
            {
                return;
    ...
    KDTree2d::KDTree2d( std::vector<KDTreeEntry2d> entries ) : entries_( std::move( entries ) ) {}

    void KDTree2d::Clear() { entries_.clear(); }
    ...
    bool KDTree2d::Remove( std::size_t id )
    {
        const auto it = std::remove_if( entries_.begin(), entries_.end(),
                                        [id]( const KDTreeEntry2d &entry ) { return entry.id == id; } );
        if( it == entries_.end() )
        {
            return false;
        }
    ...
    std::size_t KDTree2d::Size() const { return entries_.size(); }

    bool KDTree2d::IsEmpty() const { return entries_.empty(); }
    ...
    bool KDTree2d::Contains( std::size_t id ) const { return Find( id ) != nullptr; }
    ...
    std::vector<std::size_t> KDTree2d::QueryWithinDistance( const Point2d &point,
    ...
    std::string KDTree2d::DebugString() const
    {
        ...
        stream << "KDTree2d{size=" << Size() << ", valid=" << ( IsValid() ? "true" : "false" ) << "}";
        return stream.str();
    }

    const std::vector<KDTreeEntry2d> &KDTree2d::Entries() const { return entries_; }

    std::vector<KDTreeEntry2d> &KDTree2d::Entries() { return entries_; }

    const std::vector<KDTreeEntry2d> &KDTree2d::Data() const { return Entries(); }

    std::vector<KDTreeEntry2d> &KDTree2d::Data() { return Entries(); }
}  // namespace Geometry
