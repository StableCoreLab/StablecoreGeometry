...
namespace Geometry
{
    ...
    bool Rectangle2d::IsValid() const
    {
        return center.IsValid() && std::isfinite( width ) && std::isfinite( height ) &&
               std::isfinite( angleRadians ) && width > 0.0 && height > 0.0;
    }

    double Rectangle2d::Area() const { return IsValid() ? width * height : 0.0; }

    double Rectangle2d::Perimeter() const { return IsValid() ? 2.0 * ( width + height ) : 0.0; }
    ...
    Polygon2d Rectangle2d::ToPolygon() const
    {
        ...
        return Polygon2d( Polyline2d( std::vector<Point2d>( corners.begin(), corners.end() ),
    ...
    std::string Rectangle2d::DebugString() const
    {
        ...
        stream << "Rectangle2d{center=" << center.DebugString() << ", width=" << width
               << ", height=" << height << ", angleRadians=" << angleRadians << "}";
        return stream.str();
    }
}  // namespace Geometry
