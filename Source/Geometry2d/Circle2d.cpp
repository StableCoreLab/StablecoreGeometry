...
namespace Geometry
{
    Circle2d::Circle2d( const Point2d &center, double radius ) : center( center ), radius( radius ) {}
    ...
    Box2d Circle2d::Bounds() const
    {
        ...
        return Box2d::FromMinMax( Point2d{ center.x - radius, center.y - radius },
                                  Point2d{ center.x + radius, center.y + radius } );
    }

    Point2d Circle2d::PointAtAngle( double angleRadians ) const
    {
        return Point2d{ center.x + radius * std::cos( angleRadians ),
