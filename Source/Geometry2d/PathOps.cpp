...
namespace Geometry
{
    namespace
    {
        ...
        [[nodiscard]] Point2d IntersectInfiniteLines( const Point2d &a1, const Point2d &a2,
        ...
        [[nodiscard]] Polyline2d ClipPolylineLength( const Polyline2d &polyline, double startLength,
                                                     double endLength )
        {
            ...
            for( std::size_t i = 0; i + 1 < polyline.PointCount(); ++i )
            {
                ...
                if( points.empty() ||
                    !points.back().AlmostEquals( startPoint, Geometry::kPathOpsDefaultEpsilon ) )
                {
                    points.push_back( startPoint );
                }
                if( points.empty() ||
                    !points.back().AlmostEquals( endPoint, Geometry::kPathOpsDefaultEpsilon ) )
                {
        ...
        [[nodiscard]] Polygon2d ClipPolygonHalfPlane( const Polygon2d &polygon,
                                                      const LineSegment2d &cutter, bool keepLeft )
        {
            ...
            for( const Point2d &current : input )
            {
                ...
                if( currentInside )
                {
                    if( !prevInside )
                    {
                        output.push_back( IntersectInfiniteLines( prev, current, cutter.startPoint,
                                                                  cutter.endPoint ) );
                    }
                ...
                {
                    output.push_back(
                        IntersectInfiniteLines( prev, current, cutter.startPoint, cutter.endPoint ) );
                }
        ...
        [[nodiscard]] Polygon2d BuildHalfPlaneClipPolygon( const Polygon2d &polygon,
                                                           const LineSegment2d &cutter, bool keepLeft,
                                                           double eps )
        {
            if( !polygon.IsValid() )
            ...
            const double diagonal =
                std::sqrt( ( maxPoint.x - minPoint.x ) * ( maxPoint.x - minPoint.x ) +
        ...
        [[nodiscard]] Polygon2d RebuildLargestPolygonWithCandidates(
            const MultiPolyline2d &boundaries, const std::vector<double> &epsCandidates )
        {
        ...
        [[nodiscard]] MultiPolyline2d SimplifyBoundaryPolylines( const MultiPolyline2d &boundaries,
                                                                 double eps )
        {
            ...
            for( std::size_t boundaryIndex = 0; boundaryIndex < boundaries.Count(); ++boundaryIndex )
            {
                ...
                Polyline2d simplifiedBoundary(
                    std::move( simplifiedPoints ),
                    isClosed ? PolylineClosure::Closed : PolylineClosure::Open );
        ...
        [[nodiscard]] std::vector<RawSegment> CollectRawSegments( const MultiPolyline2d &polylines,
                                                                  double eps )
        {
        ...
        [[nodiscard]] std::size_t FindVertexIndex( const std::vector<Point2d> &vertices,
        ...
        [[nodiscard]] VertexGraph2d BuildVertexGraph( const std::vector<RawSegment> &segments,
                                                      double eps )
        {
            VertexGraph2d graph;
        ...
        [[nodiscard]] double ComputeRepairTolerance( const std::vector<RawSegment> &segments,
                                                     double eps )
        {
        ...
        void AutoCloseDanglingEndpoints( std::vector<RawSegment> &segments, double repairTol,
                                         double eps )
        {
            ...
            for( std::size_t i = 0; i < endpoints.size(); ++i )
            {
                ...
                if( bestIndex != static_cast<std::size_t>( -1 ) )
                {
                    ...
                    used[bestIndex] = true;
                    segments.push_back(
                        RawSegment{ first, graph.vertices[endpoints[bestIndex]], true } );
                }
            }
        }

        void AutoExtendDanglingEndpoints( std::vector<RawSegment> &segments, double repairTol,
                                          double eps )
        {
            const VertexGraph2d graph = BuildVertexGraph( segments, eps );
            ...
            for( std::size_t vertexIndex = 0; vertexIndex < graph.vertices.size(); ++vertexIndex )
            {
                ...
                for( std::size_t segmentIndex = 0; segmentIndex < originalCount; ++segmentIndex )
                {
                    ...
                    if( endpoint.AlmostEquals( segment.start, eps ) ||
                        endpoint.AlmostEquals( segment.end, eps ) )
                    {
                        continue;
                    }

                    const SegmentProjection2d projection = ProjectPointToLineSegment(
                        endpoint, LineSegment2d( segment.start, segment.end ), true );
                    ...
                    if( projection.distanceSquared > repairTol * repairTol ||
                        projection.distanceSquared <= eps * eps )
                    {
                        continue;
                    }
        ...
        [[nodiscard]] std::vector<RawSegment> PruneDanglingSegments( std::vector<RawSegment> segments,
                                                                     double eps )
        {
            bool changed = true;
            while( changed )
                               ...
                               std::unordered_set<std::uint64_t> &edgeKeys, double eps )
        {
            for( const RawSegment &segment : segments )
            {
                ...
                edges.push_back( DirectedEdge{ from, to, forwardIndex + 1,
                                               std::atan2( forward.y, forward.x ), forward.Length(),
                                               segment.synthetic, false } );
                edges.push_back( DirectedEdge{ to, from, forwardIndex,
                                               std::atan2( backward.y, backward.x ), backward.Length(),
                                               segment.synthetic, false } );
        ...
        [[nodiscard]] double ComputeAreaTolerance( const std::vector<RawSegment> &segments,
                                                   double repairTol, double eps )
        {
            if( segments.empty() )
            {
        ...
        [[nodiscard]] bool RejectRingCandidate( const RingCandidate &candidate, double repairTol,
                                                double areaTol, double eps )
        {
        ...
        [[nodiscard]] double ComputeRingCandidateScore( double area, double syntheticPerimeter,
        ...
        [[nodiscard]] std::vector<RingCandidate> ExtractCandidateRings(
            ...
            const std::vector<std::vector<std::size_t>> &outgoing, double repairTol, double areaTol,
            double eps )
        {
            ...
            for( std::size_t start = 0; start < edges.size(); ++start )
            {
                ...
                const double score =
                    ComputeRingCandidateScore( area, syntheticPerimeter, branchVertexCount,
                                               syntheticBranchVertexCount, repairTol, eps );
                const RingCandidate candidate{ ring,
                                               area,
                                               perimeter,
                                               syntheticPerimeter,
                                               branchVertexCount,
            ...
            std::stable_sort(
                rings.begin(), rings.end(), []( const RingCandidate &lhs, const RingCandidate &rhs ) {
        ...
        [[nodiscard]] std::size_t ComputeDepth( const std::vector<std::size_t> &parents,
                                                std::size_t index )
        {
            std::size_t depth = 0;
        ...
        [[nodiscard]] MultiPolygon2d BuildFilledPolygonsFromCandidateRings(
            const std::vector<RingCandidate> &rings, double eps )
        {
    ...
    Polygon2d NormalizePolygonByLines( const Polygon2d &polygon, double eps )
    {
        ...
        const std::vector<double> epsCandidates{
            ...
            std::max( eps, Geometry::kPathOpsRebuildFallbackEpsilon ), std::max( eps, 1e-7 )
        };
        ...
        const double simplifyEps = std::max(
            Geometry::kPathOpsAreaEpsilon, 8.0 * std::max( eps, Geometry::kPathOpsComparisonEpsilon ) );
