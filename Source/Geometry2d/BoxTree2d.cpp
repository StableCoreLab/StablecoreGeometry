...
namespace Geometry
{
    namespace
    {
        ...
        [[nodiscard]] Box2d ComputeBounds( const std::vector<BoxTreeEntry2d> &entries,
        ...
        [[nodiscard]] std::unique_ptr<BoxNode> BuildBoxTree( const std::vector<BoxTreeEntry2d> &entries,
                                                             ...
                                                             int depth )
        {
            ...
            std::sort( indices.begin(), indices.end(),
                       [&entries, axis]( std::size_t lhs, std::size_t rhs ) {
                           return CenterCoordinate( entries[lhs].box, axis ) <
                                  CenterCoordinate( entries[rhs].box, axis );
                       } );
            ...
            std::vector<std::size_t> rightIndices( indices.begin() + static_cast<std::ptrdiff_t>( mid ),
                                                   indices.end() );
        ...
        void QueryBox( const BoxNode *node, const std::vector<BoxTreeEntry2d> &entries, const Box2d &box,
                       double eps, std::vector<std::size_t> &result )
        {
        ...
        void QueryPoint( const BoxNode *node, const std::vector<BoxTreeEntry2d> &entries,
                         const Point2d &point, double eps, std::vector<std::size_t> &result )
        {
    ...
    BoxTree2d::BoxTree2d( std::vector<BoxTreeEntry2d> entries ) : entries_( std::move( entries ) ) {}

    void BoxTree2d::Clear() { entries_.clear(); }
    ...
    bool BoxTree2d::Remove( std::size_t id )
    {
        const auto it = std::remove_if( entries_.begin(), entries_.end(),
                                        [id]( const BoxTreeEntry2d &entry ) { return entry.id == id; } );
        if( it == entries_.end() )
        {
            return false;
        }
    ...
    std::size_t BoxTree2d::Size() const { return entries_.size(); }

    bool BoxTree2d::IsEmpty() const { return entries_.empty(); }
    ...
    bool BoxTree2d::Contains( std::size_t id ) const { return Find( id ) != nullptr; }
    ...
    std::string BoxTree2d::DebugString() const
    {
        ...
        stream << "BoxTree2d{size=" << Size() << ", valid=" << ( IsValid() ? "true" : "false" ) << "}";
        return stream.str();
    }

    const std::vector<BoxTreeEntry2d> &BoxTree2d::Entries() const { return entries_; }

    std::vector<BoxTreeEntry2d> &BoxTree2d::Entries() { return entries_; }

    const std::vector<BoxTreeEntry2d> &BoxTree2d::Data() const { return Entries(); }

    std::vector<BoxTreeEntry2d> &BoxTree2d::Data() { return Entries(); }
}  // namespace Geometry
