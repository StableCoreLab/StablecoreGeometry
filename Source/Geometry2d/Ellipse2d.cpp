...
namespace Geometry
{
    Ellipse2d::Ellipse2d( const Point2d &center, double majorRadius, double minorRadius,
                          double angleRadians ) :
        center( center ),
    ...
    double Ellipse2d::Perimeter() const
    {
        ...
        const double h = std::pow( ( majorRadius - minorRadius ), 2.0 ) /
                         std::pow( ( majorRadius + minorRadius ), 2.0 );
    ...
    Polygon2d Ellipse2d::ToPolygon( std::size_t segmentCount ) const
    {
        ...
        {
            const double t = 2.0 * std::numbers::pi_v<double> * static_cast<double>( i ) /
    ...
    std::string Ellipse2d::DebugString() const
    {
        ...
        stream << "Ellipse2d{center=" << center.DebugString() << ", majorRadius=" << majorRadius
