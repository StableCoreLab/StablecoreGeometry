...
namespace Geometry
{
    struct Triangle3d
    {
        ...
        [[nodiscard]] Vector3d Normal() const { return Cross( b - a, c - a ); }

        [[nodiscard]] double Area() const { return 0.5 * Normal().Length(); }

        [[nodiscard]] Point3d Centroid() const
        {
            return Point3d{ ( a.x + b.x + c.x ) / 3.0, ( a.y + b.y + c.y ) / 3.0,
                            ( a.z + b.z + c.z ) / 3.0 };
        }

        [[nodiscard]] Plane Plane() const { return Geometry::Plane::FromPointAndNormal( a, Normal() ); }

        [[nodiscard]] std::string DebugString() const
        {
            std::ostringstream stream;
            stream << "Triangle3d{a=" << a.DebugString() << ", b=" << b.DebugString()
                   << ", c=" << c.DebugString() << "}";
            return stream.str();
        }
    };
