...
namespace Geometry
{
    ...
    public:
        ...
        RuledSurface( std::shared_ptr<Curve3d> first, std::shared_ptr<Curve3d> second,
        ...
        [[nodiscard]] static RuledSurface FromCurves( const Curve3d &first, const Curve3d &second,
                                                      Intervald vRange = Intervald{ 0.0, 1.0 } )
        {
        ...
        [[nodiscard]] Intervald VRange() const override { return vRange_; }
        ...
        [[nodiscard]] Point3d PointAt( double u, double v ) const override
        {
            ...
            const double normalized = vRange_.Length() <= Geometry::kDefaultEpsilon
                                          ? 0.0
                                          : ( v - vRange_.min ) / vRange_.Length();
                                              ...
                                              int derivativeOrder = 1 ) const override
        {
            ...
            if( eval.derivativeOrder >= 1 && first_ != nullptr && second_ != nullptr )
            {
                ...
                eval.derivativeU = firstEval.firstDerivative +
                                   ( secondEval.firstDerivative - firstEval.firstDerivative ) * weight;
        ...
        [[nodiscard]] const Curve3d *FirstCurve() const { return first_.get(); }

        [[nodiscard]] const Curve3d *SecondCurve() const { return second_.get(); }
