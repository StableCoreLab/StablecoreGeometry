...
TEST(Healing3dGapTest, AggressiveShellRepairPolicyRemainsOpen)
{
    ...
    (void)healing;
    GTEST_SKIP() << "Known 3D gap: general topology-changing aggressive shell repair policy beyond recoverable planar open-shell subsets (single/multi-face, holed, multi-shell, mixed closed/open-shell, partial mixed-shell repair, three-shell deterministic mixed behavior, three-shell+trim-backfill interplay, three-shell with eligible multi-face open-sheet, three-shell with eligible holed-shell missing trims, three-shell with eligible multi-face holed+missing-trims, three-shell with eligible multi-face both-trims-missing, three-shell with eligible support-plane mismatch composition) is not implemented.";
}
