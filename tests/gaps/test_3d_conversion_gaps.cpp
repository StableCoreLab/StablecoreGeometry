...
TEST(Conversion3dGapTest, GeneralNonPlanarPolyhedronToBrepRepairRemainsOpen)
{
    ...
    (void)mesh;
    GTEST_SKIP() << "Known 3D gap: robust non-planar polyhedron->Brep repair beyond affine-planar + support-plane-refit (including closed-shell shared-topology subset via triangular faces [closed tetrahedron], support-plane-mismatch quad shared-edge-chain shared-topology subset, shared-chain mixed-content full-composition subset, dual outer/hole duplicate-normalization composition subset, deformed-cube multi-face non-planar subsets via single-displaced-vertex and dual-displaced-vertices, near-equal shared-edge representative-average vertex-placement subset, support-mismatch near-equal shared-edge representative-average subset, support-mismatch near-equal shared-apex fan representative-average subset, support-mismatch near-equal shared-edge-chain representative-average subset, and support-mismatch near-equal shared-corner fan representative-average subset) + scale-aware tiny-loop normal fallback + mild/tiny-scale outer/hole/multi-face/mixed-content/shared-edge loop-projection + duplicate/collinear-leading normalization composition subset is still open.";
}
