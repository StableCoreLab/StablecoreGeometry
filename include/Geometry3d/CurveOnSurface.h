...
namespace Geometry
{
    ...
    public:
        ...
        CurveOnSurface( std::shared_ptr<Surface> supportSurface, Polyline2d uvCurve ) :
            ...
            uvCurve_( std::move( uvCurve ) )
        {
        }

        [[nodiscard]] bool IsValid( const GeometryTolerance3d &tolerance = {} ) const
        {
            return supportSurface_ != nullptr && supportSurface_->IsValid( tolerance ) &&
                   uvCurve_.IsValid();
        }

        [[nodiscard]] const Surface *SupportSurface() const { return supportSurface_.get(); }

        [[nodiscard]] const Polyline2d &UvCurve() const { return uvCurve_; }

        [[nodiscard]] std::size_t PointCount() const { return uvCurve_.PointCount(); }

        [[nodiscard]] Point2d UvPointAt( std::size_t index ) const { return uvCurve_.PointAt( index ); }
