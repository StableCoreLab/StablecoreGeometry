...
namespace Geometry
{
    ...
    public:
        ...
        NurbsCurve3d( int degree, std::vector<Point3d> controlPoints, std::vector<double> knots,
                      bool periodic = false ) :
            degree_( degree ),
        ...
        {
            if( degree_ < 1 || controlPoints_.size() < 2 ||
        ...
        {
            return IsValid() ? Intervald{ knots_[static_cast<std::size_t>( degree_ )],
        ...
        {
            return controlPoints_.size() >= 2 && controlPoints_.front().AlmostEquals(
                                                     controlPoints_.back(), tolerance.distanceEpsilon );
        }
        ...
        [[nodiscard]] CurveEval3d Evaluate( double parameter, int derivativeOrder = 1 ) const override
        {
            ...
            if( derivativeOrder >= 1 && controlPoints_.size() >= 2 )
            {
                ...
                const double step = std::max(
                    range.Length() /
                        static_cast<double>( std::max<std::size_t>( controlPoints_.size() * 8, 8 ) ),
                ...
                const double denom =
                    std::max( nextParameter - prevParameter, Geometry::kDefaultEpsilon );
    ...
    private:
        [[nodiscard]] double Basis( std::size_t index, int degree, double parameter ) const
        {
            ...
            {
                left = ( ( parameter - knots_[index] ) / leftDenom ) *
                       Basis( index, degree - 1, parameter );
            }
            ...
            {
                right = ( ( knots_[index + static_cast<std::size_t>( degree ) + 1] - parameter ) /
                          rightDenom ) *
