...
namespace Geometry
{
    ...
    public:
        ...
        [[nodiscard]] virtual CurveEval3d Evaluate( double parameter,
                                                    int derivativeOrder = 1 ) const = 0;
        [[nodiscard]] virtual Box3d Bounds() const = 0;
        [[nodiscard]] virtual std::unique_ptr<Curve3d> Clone() const = 0;

        [[nodiscard]] double StartParameter() const { return ParameterRange().min; }

        [[nodiscard]] double EndParameter() const { return ParameterRange().max; }

        [[nodiscard]] double StartT() const { return StartParameter(); }

        [[nodiscard]] double EndT() const { return EndParameter(); }
    };
}  // namespace Geometry
