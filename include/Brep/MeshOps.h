...
namespace Geometry
{
    ...
    {
        std::array<std::size_t, 3> adjacentTriangles{ std::size_t( -1 ), std::size_t( -1 ),
                                                      std::size_t( -1 ) };
    ...
    struct GEOMETRY_API MeshBoundaryLoop3d
    {
        ...
        [[nodiscard]] bool IsValid() const { return vertexIndices.size() >= ( closed ? 3U : 2U ); }
    };
    ...
    struct GEOMETRY_API MeshShell3d
    {
        ...
        [[nodiscard]] bool IsValid() const { return !triangleIndices.empty(); }
    };
