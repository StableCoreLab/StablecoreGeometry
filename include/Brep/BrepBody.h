...
namespace Geometry
{
    ...
    public:
        BrepBody() = default;
        BrepBody( std::vector<BrepVertex> vertices, std::vector<BrepEdge> edges,
        ...
        [[nodiscard]] std::size_t ShellCount() const { return shells_.size(); }
        ...
        [[nodiscard]] BrepVertex VertexAt( std::size_t index ) const { return vertices_.at( index ); }

        [[nodiscard]] BrepEdge EdgeAt( std::size_t index ) const { return edges_.at( index ); }

        [[nodiscard]] BrepShell ShellAt( std::size_t index ) const { return shells_.at( index ); }

        [[nodiscard]] const std::vector<BrepVertex> &Vertices() const { return vertices_; }

        [[nodiscard]] const std::vector<BrepEdge> &Edges() const { return edges_; }

        [[nodiscard]] const std::vector<BrepShell> &Shells() const { return shells_; }
        ...
        [[nodiscard]] std::string DebugString() const
        {
            ...
            stream << "BrepBody{vertexCount=" << VertexCount() << ", edgeCount=" << EdgeCount()
