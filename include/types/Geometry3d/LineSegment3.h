...
namespace Geometry
{
    struct LineSegment3d
    {
        ...
        [[nodiscard]] static LineSegment3d FromStartEnd( const Point3d &startValue,
                                                         const Point3d &endValue )
        {
        ...
        [[nodiscard]] bool IsValid( double eps = kDefaultEpsilon ) const
        {
            return startPoint.IsValid() && endPoint.IsValid() &&
                   ( endPoint - startPoint ).Length() > eps;
        }

        [[nodiscard]] double Length() const { return ( endPoint - startPoint ).Length(); }
