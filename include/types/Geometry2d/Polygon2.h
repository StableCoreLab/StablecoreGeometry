...
namespace Geometry
{
    ...
    public:
        ...
        explicit Polygon2( PolylineType outerRing ) : outerRing_( std::move( outerRing ) ) {}
        ...
        [[nodiscard]] const PolylineType &OuterRing() const { return outerRing_; }
    ...
    private:
            ...
            const PolylineType &ring )
        {
            ...
            return { Point2<MeasureType>( moment.firstMomentX / moment.signedArea,
        ...
        static void AccumulateLineSegmentMoment( const PointType &startPoint, const PointType &endPoint,
                                                 RingMoment &total )
        {
            ...
            total.firstMomentX +=
                ( y1 - y0 ) * ( x0 * x0 + x0 * x1 + x1 * x1 ) / static_cast<MeasureType>( 6 );
            total.firstMomentY +=
                ( x0 - x1 ) * ( y0 * y0 + y0 * y1 + y1 * y1 ) / static_cast<MeasureType>( 6 );
        }
        ...
        static void AccumulateArcSegmentMoment( const Segment2<T> &segment, RingMoment &total )
        {
            ...
            const MeasureType sinStart =
                static_cast<MeasureType>( std::sin( static_cast<double>( startAngle ) ) );
            const MeasureType cosStart =
                static_cast<MeasureType>( std::cos( static_cast<double>( startAngle ) ) );
            const MeasureType sinEnd =
                static_cast<MeasureType>( std::sin( static_cast<double>( endAngle ) ) );
            const MeasureType cosEnd =
                static_cast<MeasureType>( std::cos( static_cast<double>( endAngle ) ) );

            total.signedArea += ( cx * radius * ( sinEnd - sinStart ) -
                                  cy * radius * ( cosEnd - cosStart ) + radius * radius * sweep ) /
        ...
        [[nodiscard]] static MeasureType ArcFirstMomentX( MeasureType centerX, MeasureType radius,
                                                          MeasureType startAngle, MeasureType endAngle )
        {
            return ArcFirstMomentXPrimitive( centerX, radius, endAngle ) -
                   ArcFirstMomentXPrimitive( centerX, radius, startAngle );
        }

        [[nodiscard]] static MeasureType ArcFirstMomentXPrimitive( MeasureType centerX,
                                                                   MeasureType radius,
                                                                   MeasureType angle )
        {
            const MeasureType sinAngle =
                static_cast<MeasureType>( std::sin( static_cast<double>( angle ) ) );
            const MeasureType cosAngle =
                static_cast<MeasureType>( std::cos( static_cast<double>( angle ) ) );
            return ( centerX * centerX * radius * sinAngle +
                     centerX * radius * radius * ( angle + sinAngle * cosAngle ) +
                     radius * radius * radius *
                         ( sinAngle -
                           sinAngle * sinAngle * sinAngle / static_cast<MeasureType>( 3 ) ) ) /
                   static_cast<MeasureType>( 2 );
        }

        [[nodiscard]] static MeasureType ArcFirstMomentY( MeasureType centerY, MeasureType radius,
                                                          MeasureType startAngle, MeasureType endAngle )
        {
        ...
        [[nodiscard]] static MeasureType ArcFirstMomentYPrimitive( MeasureType centerY,
                                                                   MeasureType radius,
                                                                   MeasureType angle )
        {
            const MeasureType sinAngle =
            ...
            return ( -centerY * centerY * radius * cosAngle +
                     ...
                     radius * radius * radius *
                         ( -cosAngle +
                           cosAngle * cosAngle * cosAngle / static_cast<MeasureType>( 3 ) ) ) /
                   static_cast<MeasureType>( 2 );
        }
