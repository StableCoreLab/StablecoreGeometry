...
namespace Geometry
{
    ...
    public:
        ...
        constexpr Box2( const Point2<T> &minPoint, const Point2<T> &maxPoint ) :
            min_( minPoint ),
            max_( maxPoint ),
            valid_( IsOrdered( minPoint, maxPoint ) )
        {
        }

        [[nodiscard]] static constexpr Box2 FromMinMax( const Point2<T> &minPoint,
        ...
        [[nodiscard]] constexpr const Point2<T> &MinPoint() const { return min_; }

        [[nodiscard]] constexpr const Point2<T> &MaxPoint() const { return max_; }
        ...
        [[nodiscard]] constexpr Point2<MeasureType> Center() const
        {
            ...
            return Point2<MeasureType>(
                ( static_cast<MeasureType>( min_.x ) + static_cast<MeasureType>( max_.x ) ) /
                    static_cast<MeasureType>( 2 ),
                ( static_cast<MeasureType>( min_.y ) + static_cast<MeasureType>( max_.y ) ) /
                    static_cast<MeasureType>( 2 ) );
        }
        ...
        [[nodiscard]] constexpr bool AlmostEquals( const Box2 &other,
