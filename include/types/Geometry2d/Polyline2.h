...
namespace Geometry
{
    ...
    public:
        ...
        explicit Polyline2( PolylineClosure2 closure ) : closure_( closure ) {}

        Polyline2( std::vector<SegmentPtr> segments,
                   PolylineClosure2 closure = PolylineClosure2::Open ) :
        ...
        [[nodiscard]] PolylineClosure2 Closure() const { return closure_; }

        [[nodiscard]] bool IsClosed() const { return closure_ == PolylineClosure2::Closed; }
                                               ...
                                               bool clampToPath = false ) const
        {
            ...
            if( distanceFromStart > totalLength )
            {
                return segments_.back()->PointAtLength(
                    distanceFromStart - PrefixLength( segments_.size() - 1 ), false );
            }
        ...
        [[nodiscard]] double ParameterAtLength( LengthType distanceFromStart,
                                                bool clampToPath = false ) const
        {
            const LengthType totalLength = Length();
            if( totalLength <= LengthType{} )
